...
| fail-fast 预检 | 🆕 **先本地校验服务端 DTO 的真实正则（7 个字段）** → 影子表在不在 / `testsim%` 是否为 0 / 库存够不够 / 地址模板在不在 / 目录非空 / 🆕 **9 张表的 `data_source` 列是否已就位**（列缺 = 迁移没跑 → 拒绝执行） | §〇.1 D7 / §2.2.9 / §〇.2 §G |
