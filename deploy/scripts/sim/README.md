...
> **来源方案**：**[[Python模拟数据与数据隔离方案]]**（第一层正文；原《Python 模拟数据 + AI 并发测试方案》2026-09-11 切割，第二层 → [[AI并发测试方案]]）
> （规范设计定稿 2026-09-10 + **实施前复核 2026-09-11 §〇.1**，本节与方案文档**双向对应**）**—— 本文中所有「方案 §X」均指 A 册，除非另注**
> **对应 TODO**：`docs/评估报告/TODO文件.md` **#48**
> **本目录只做"第一层：造数"**；第二层 AI 并发压测与 mock LLM 见 **[[AI并发测试方案]] §五**（原方案 §3.5，切割后重新编号）。
...
### 4.9 🆕 跑完必做：按批次复核（`--verify`，2026-09-12 新增）

```bash
export SIM_DB_PASSWORD='<老机 MySQL root 密码>'
python3 simulate_data.py --verify --batch sim_YYYYMMDD_HHMM   # 有问题退出码 1
```
...
| 🆕 **#67② 正式造数（2026-09-12，1 天 × 1000 行为 @ `--qps 5`）** | ✅ **全绿**：`浏览 810 / 加购 139 / 下单 51（已支付 51，支付失败 0）/ 失败 0` · 实测 **4.98 QPS**（12:34:10→12:37:31 = 3 分 21 秒）· 新建 **125** 用户（号段 `1390100`）· 登记 **366** 实体 · 标记 **539** 行 · 批次 `sim_20260912_1233` |
| 🆕 **按批次独立复核（9 表）** | ✅ **漏标 0**（含 `oms_order_item` 按父订单兜底 0）· `testsim0912*` 未标 0 · 悬空登记 3 条 `oms_cart`（下单清购物车所致，**无残留**） |
| 🆕 **`--verify`（新入口）验收** | ✅ `--verify sim_20260912_1233` → 全表 ✅ / **0 问题 / exit=0**；`--verify sim_20260911_1930` → 仅剩 1 条真问题（`ums_login_log` 227 行 = **回填之后**新产生的登录，见方案 F8） |
| 🆕 **预检挡下一次真实碰撞** | ✅ `1390009` 段被上一批占 **20** 个（`13900090001..`）→ 预检 **fail-fast** 并列出空闲段；改用 `1390100`（独立复核 **0 占用**）后通过 |
| 未验证 | 🟡 更新：**写链路已全部真跑过**（注册/登录/加购/下单/支付/**回填**/清理 → 见上）；仍未做的：**秒杀真跑**（④）· **`--clean` 的 Redis 部分**（⑤）· **`sim_baseline` 写入**（⑥）· **`sim_batch.dump_file` 落盘**（历次 7 个批次全为空） |

## 九、关联文档

[[Python模拟数据与数据隔离方案]]（§〇.1 复核 · §2.2 隔离 · §2.5 清理清单 · §六 展示 SOP）· [[AI并发测试方案]]（§一~§五 压测剧本）
· [[TODO文件]]#48 · [[TODO已完成]]#29（cron 备份）/ #47（恢复演练）
· [[TODO第三批实现与原理-1]] §一 切割索引（**原 §四** 模拟数据选型 的去向）· [[问题解决--生产造数的数据隔离与复核方法]]（已提炼的「一类问题」）
