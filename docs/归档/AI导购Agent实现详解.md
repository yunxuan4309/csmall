...
> 📦 **归档说明**：本文是 **#32 的实现说明书 / 排障底稿**，而 **#32 已全部完成并归档**（→ [[TODO已完成]] §十六/§十七），其核心内容也已**提炼为「一类问题」文档** → [[问题解决--LLM链路的契约漂移与分层降级]]。**归档 ≠ 内容失效**：**三步排障法（§7.4）· 日志关键行（§7.1）· 审计键回放（§7.3）· 开关语义与三级回滚（§八）** 仍长期可用，只是不再作为"待实施文档"维护。**当前状态一律以 [[TODO文件]] 为准。**
