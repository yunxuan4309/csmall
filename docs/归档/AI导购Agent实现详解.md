# AI 导购 Agent 实现详解（Function Calling + ReAct · 手写薄编排）· **已归档**

> **创建日期**: 2026-09-10 ┃ **归档**: 2026-09-11
> 📦 **归档说明**：本文是 **#32 的实现说明书 / 排障底稿**，而 **#32 已全部完成并归档**（→ [[TODO已完成-明细-2026-09-12]] §十六/§十七），其核心内容也已**提炼为「一类问题」文档** → [[问题解决--LLM链路的契约漂移与分层降级]]。**归档 ≠ 内容失效**：**三步排障法（§7.4）· 日志关键行（§7.1）· 审计键回放（§7.3）· 开关语义与三级回滚（§八）** 仍长期可用，只是不再作为"待实施文档"维护。**当前状态一律以 [[TODO文件]] 为准。**
...
> **一句话**: **不引框架，用 OpenAI 兼容的 Function Calling 手写"一个循环 + 一份工具清单"**，把项目已有的会话/预算/闸门/SSE/检索/降级全部复用。为什么不上 Spring AI → 见 [[问题解决--引入新依赖前的影响面取证]] §二（结论：卡 Boot 代际，本项目 3.2.5 低于其支持下限）。
...
| # | 验证 | 结论 |
|---|---|---|
| A~J | 见 [[问题解决--LLM链路的契约漂移与分层降级]] §七 | 思考开关生效 / **400 规则复现** / tools 可触发 / **tools 与 response_format 互斥** / 两轮循环收敛 / SSE + 思考安全 |
...
| 决策 | 选了什么 | 替代方案 | 依据 |
|---|---|---|---|
| Agent 框架 | **手写薄编排** | Spring AI / LangChain4j | Spring AI 支持下限 Boot 3.4，本项目 3.2.5（全反应堆 13 模块继承根 pom）→ 见 [[问题解决--引入新依赖前的影响面取证]] §二 |
...
- **选型过程 + 13 格实验 + 小插曲**：[[问题解决--引入新依赖前的影响面取证]]（§二 为何手写 + 影响面实测）· [[问题解决--LLM链路的契约漂移与分层降级]]（§六 模型可配化 / §七 13 格实验）· [[问题解决--服务注册与网关路由]]（小插曲 nginx 502）
- **模型契约与可配化**：[[AI模型名停用风险与thinking参数改造方案]]（#58：thinking 开关 + 档位/任务两层）
- **状态源**：[[TODO文件]] #32 / #60 / #61 / #62；已完成归档 [[TODO已完成-明细-2026-09-12]] §十六
