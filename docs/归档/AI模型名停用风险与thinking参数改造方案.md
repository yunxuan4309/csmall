...
> 📦 **归档说明**：本方案对应 **#58，已实施 + 已部署生产并验证通过**（→ [[TODO已完成]] §十四），其核心内容（契约漂移 / 死参数·死配置·死代码）已**提炼为「一类问题」文档** → [[问题解决--LLM链路的契约漂移与分层降级]]。**归档 ≠ 内容失效**：**§3.1 的 10 格实测实验、§4.5 模型配置可配化、§4.6~4.7 为什么暂不引入 Spring AI** 仍长期有参考价值（该文原本就登记为"已完成但保留"）。**当前状态一律以 [[TODO文件]] 为准。**
