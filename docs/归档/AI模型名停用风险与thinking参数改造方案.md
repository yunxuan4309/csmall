# AI 模型名停用风险与 `thinking` 参数改造方案 · **已归档**

> **创建日期**: 2026-09-10 ┃ **归档**: 2026-09-11
> 📦 **归档说明**：本方案对应 **#58，已实施 + 已部署生产并验证通过**（→ [[TODO已完成-明细-2026-09-12]] §十四），其核心内容（契约漂移 / 死参数·死配置·死代码）已**提炼为「一类问题」文档** → [[问题解决--LLM链路的契约漂移与分层降级]]。**归档 ≠ 内容失效**：**§3.1 的 10 格实测实验、§4.5 模型配置可配化、§4.6~4.7 为什么暂不引入 Spring AI** 仍长期有参考价值（该文原本就登记为"已完成但保留"）。**当前状态一律以 [[TODO文件]] 为准。**
...
> 🧩 **已提炼（2026-09-11）**：本文件的「§一 问题（契约漂移）+ §二 根因 + §五 三个死参数/死配置/死代码」已提炼为「一类问题」文档 → **[[问题解决--LLM链路的契约漂移与分层降级]]**（**原文件保留不动**，仅互链；本篇仍是完整的改造方案 + 实施记录 + 话术）
