...
> **📦 归档日期：2026-09-10** —— 归档原因：① 这是**开发计划 / 设计文档**（不是"问题解决"文档，与 `docs/问题解决/` 其余 30 篇性质不同）；② **四阶段已 100% 完成**（2026-07-18），计划本身失效；③ 文内"**当前配置项清单 / 接口清单**"**已发生漂移**（见下方对照）。
> ✅ **仍有参考价值、且不会漂移的部分**：架构决策、5 阶段搜索流水线、ES 索引结构、**Embedding 方案选型**（DeepSeek `/v1/embeddings` 404 → 硅基流动 BGE-M3）。
> ⚠️ **2026-09-10 追加更正**：本文多处写 "BGE-M3 **免费**（2000 万 tokens）" —— **该表述已不准确**：实测硅基流动免费额度**不足以调用**（`POST /v1/embeddings` 返 **402 account balance is insufficient**，新 key 认证本身有效）→ 见 [[TODO文件]] **#59**。选型理由应改为"**bge-m3 开源、可本地自建；托管商按量计费很低**"。

---

## ⚠️ 归档前复核：文中"当前清单"已漂移（**清单类内容一律以代码为准**）

| 文中写（2026-07-18）| 2026-09-10 实际 | 说明 |
|---|---|---|
| `chat-model: deepseek-v4-flash` | **`deepseek-chat`** | 2026-09-08 为解决"reasoning 模型把 `content` 挤空"而做**模型分工**；⚠️ 该名官方已公告停用 → **正是 TODO #58 的隐患来源**（见 [[AI模型名停用风险与thinking参数改造方案]]）|
| `compare-model: deepseek-v4-flash` | 名义仍是它，但**零代码引用**（死配置）| 见 [[问题解决--搜索双索引与降级分层]] |
| `timeout: 15000` | **60000**（60s）| 15s 对 LLM 不够 |
| `max-tokens: 2000` | **3000** | — |
| 接口清单 8 个 | **缺 `/ai/search`、`/ai/suggest`、`/ai/related`** | 第四阶段语义搜索的**核心接口**，本清单未收录 |

> **关联（归档时）**：[[问题解决--搜索双索引与降级分层]]（#33 已把搜索改造为"统一索引 + 只读降级层"，本计划描述的**双索引架构已被 A2 方案取代**）；[[AI导购Agent升级方案]]（⏸️ #32 未来的 Agent 升级方案，与本计划的"已完成部分"是两回事）；[[AI模型名停用风险与thinking参数改造方案]]（#58）。
