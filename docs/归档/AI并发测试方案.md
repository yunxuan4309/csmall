...
> **状态**：🟢 **首轮实测完成（2026-09-12）** —— mock / D1 透传 / Nacos 5→200 / 800 用户池 / **#62 已修** 全部就绪；**两条链路 × 20/50/100 已压完**（🆕 **2026-09-12 晚补 75/150 档 + 单次占槽**，并**定量解释了"两条链路成功 RPS 持平"** → **§十一**）（结果见 **§十**）：成功 RPS pipeline **21.4** / agent **~20.4**，20 并发档最健康（p50<1s、TTFT 100~170ms、0 硬失败）；**逐层量出两层墙**（Spring 异步池默认 8 → AI 闸门 20）。📦 **已归档（2026-09-12）**：**#67 之 ③ 已实测完成**（两条链路 × 阶梯 + 晚间深化）；原 ⏳ 待办**已全部闭环**（恢复现场 ✅ / 结果回填 ✅ §十·§十一 / **其余 7 模块同类 ASYNC·ERROR 风险 ✅ 已审计**，挂 #62「影响面」→ [[TODO已完成-明细-2026-09-12]] §十一）。方法已提炼 → [[问题解决--压测结论的可信性（先证伪干扰项与工具自身）]]。**#67 仅剩 ① 录像**，操作单 [[演示录像操作手册]] 保留
...
## 九、✅ 已实施：第一层造数脚本的改造（原「2026-09-12 登记防丢」；**2026-09-12 晚已完成**：提量/提 QPS、`--with-seckill`、秒杀后自动恢复 —— 详见 [[TODO第三批实现与原理-2]] 与 [[Python模拟数据与数据隔离方案]]）
...
> ✅ **2026-09-12 重跑成功**（用**看门狗**打法：先武装"30 分钟自动还原"，再动手；跑完立即还原并逐项核对）。**本节结论已定量解释 §十 里"两条链路成功 RPS 持平"这处不吻合。**
>
> ⚙️ **条件**：异步执行器保持**默认 8**（**没有**按上轮那样用 `SPRING_TASK_*` 放开），入口限流临时放开到 200，mock 回 **0 token**，每档 **30 s**。
>
> | 并发 | pipeline 成功 RPS | pipeline p50 | agent 成功 RPS | agent p50 | 闸门满/硬失败 |
> |---|---|---|---|---|---|
> | 20 | **11.0** | 1063ms | **8.8** | 1263ms | 0 / 0 |
> | 50 | 11.0 | 3692ms | 9.1 | 4476ms | 0 / 0 |
> | 75 | 11.3 | 5778ms | 9.2 | 7083ms | 0 / 0 |
> | 100 | 11.5 | 7888ms | 9.1 | 9718ms | 0 / 0 |
> | 150 | 11.5 | 12114ms | 9.2 | 14973ms | 0 / 0 |
>
> **① 墙在 `Spring 异步执行器（默认 8）`**：mock 侧 `peak_active=**8**`（两档都是）、成功 RPS **恒定**、**0 拒绝**、p50 随并发**线性增长** ⇒ 超过 8 并发是**排队**而不是拒绝（与 §十 的"第一层墙：异步池 8"一致）。
> **② "两链路持平"的定量解释**：
> · 反推**单次占槽**：pipeline ≈ 8 ÷ 11.2 ≈ **0.71 s**；agent ≈ 8 ÷ 9.1 ≈ **0.88 s** ⇒ 只差 **~24%**
> · 反推**每次成功的 LLM 调用数**（mock 计数）：pipeline ≈ 9728 ÷ 2075 ≈ **4.7**；agent ≈ 15029 ÷ 1747 ≈ **8.6** ⇒ **1.8×**
> ⇒ **调用数翻倍、占槽只 +24%**，因为 agent 的**额外轮次是"短调用"**（工具/JSON 轮：mock 侧 6 字/片 × 5ms，而正文是 6 字/片 × 30ms）⇒ 承载只降 ~19%（**11.2 → 9.1**），远小于"双轮 = 两倍代价"的直觉；上轮看着"持平"，是测量点落在**同一堵墙**上、被噪声掩盖了这 ~19%。
> **③ 副作用**：压测只在 AI 池用户上新增登录日志（**8081 中 800 已标 SIM ⇒ 7281 行未标**）—— 属 §〇.2 §F **F8 同类**（"回填之后新产生的行"），**可用 `--clean --batch sim_20260912_1103` 按 user_id 清掉** ✅；`SUM(sales)`/`success`/三类锁均与基线一致 ✅。
