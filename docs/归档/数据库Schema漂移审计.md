...
> 📦 **已归档（2026-09-12）**：对应待办 **#57 已关闭**（服务器只读核实 0 处需 ALTER）⇒ 归档触发条件满足；**方法已提炼**为 [[问题解决--实体库表DDL三方漂移审计]]。**归档 ≠ 内容失效**：§七 的 ALTER 清单虽**作废**，但**三方对比方法**与 **14 处差异实测证据**仍可溯源；**当前状态以 [[TODO文件]] / [[TODO已完成]] 为准。
> **最新状态（2026-09-12 更新）**：本地已全部修复；**服务器已只读核实完毕 —— 14 处差异全部是「服务器比快照新」，0 处需要 ALTER** ⇒ ~~服务器部署前需执行同款 ALTER~~（§七 的清单**作废**）。
...
> **⚠️（2026-09-12 作废，勿执行）服务器部署前需同步**：`pms_attribute_template` 的 3 列 + 4 张关联表的 `gmt_modified`，服务器数据库也需 ALTER（见文末）。
...
> 🧩 **已提炼（2026-09-11）**：本报告的三方对比方法与三类问题已提炼为「一类问题」文档 → **[[问题解决--实体库表DDL三方漂移审计]]**（**原文件保留不动**，仅互链；本报告仍是原始审计证据 + 待执行 ALTER 清单，**当前状态以 [[TODO文件]] #57 为准**）

**关联**：`docs/归档/企业级商品管理升级计划.md`（升级规划）、`docs/项目上下文文档.md` 8.4 节（实体映射遗留差异）
