...
| **#67** | **= #48 剩余部分**（~~录像~~ 🟡暂缓 · ~~正式造数~~ ✅ · ~~AI 并发压测（首轮 + 深化）~~ ✅ · ~~`--with-seckill`~~ ✅ · ~~Redis 精确清理~~ ✅ · ~~基线比对~~ ✅） | ✅ **5/6 完成（仅 ① 录像暂缓）**（白天 ②③；晚间 ④⑤⑥ + **③ 可选深化**；#68 见下） | §67 · [[AI并发测试方案]] · [[Python模拟数据与数据隔离方案]] |
...
### 67. ⏳ #48 的剩余部分（第一层 + 工具链已完成并归档 → [[TODO已完成]] §十九）
 ...
 ⏳ **2026-09-11 登记（#48 拆分而来）**：~~**#48 是两层方案，第二层「AI 并发压测」完全未开始**，剩余 6 项~~ ✅ **2026-09-12 更新**：②③④⑤⑥ 均已完成，**仅剩 ① 录像暂缓**：

① **录像**（脚本 / runbook / [[演示录像操作手册]] **均已就绪且实测过**，用户决定之后录；§6.4 三段式）

② ✅ **正式造数（2026-09-12 完成）**：批次 `sim_20260912_1233` = 1 天 × **1000 行为 @ `--qps 5`**，实测 **4.98 QPS**（3 分 21 秒）· **失败 0** · 浏览 810 / 加购 139 / 下单 51（**全支付成功**）· 新建 **125** 用户 · 登记 **366** / 标记 **539** · **按批次复核 9 表漏标 0**（方案 §五·§G **G11** · README §八）；✅ `dump_file` 已落盘并自动登记（2026-09-12，见 [[TODO文件]] §68）

③ ✅ **第二层 AI 并发压测（首轮 + 可选深化均已完成 · 2026-09-12）**：🆕 **晚间深化**：补 **75/150 档** + 量"**单次调用占槽**"⇒ 定量解释"两条链路成功 RPS 持平"（→ [[AI并发测试方案]] **§十一**）——✅ mock 已写（自测 14/14）· ✅ compose 透传 D1 + override · ✅ Nacos 热改 5→200 · ✅ 800 用户池 · ✅ 阶梯 20/50/100 **两条链路已压**（**结果见 [[AI并发测试方案]] §十**；过程中修掉 **#62**——并发下 ~50% 500）。原计划细节：`mock_llm.py` **未写**（需 `ThreadingHTTPServer` + **tool_calls 工具轮**）· compose 需加 `AI_API_BASE_URL` 透传 + override 指向 mock · Nacos 规则**备份 → 热改 `ai-chat` 阈值 → 恢复** · 构造 **N ≥ 120** 个模拟用户 token · 阶梯 20→50→100 **各压 `AI_AGENT_ENABLED=false/true` 两条链路**（产出"**Agent 双轮 vs 流水线单轮**"的承载代价）

④ ✅ **已真跑并复核（2026-09-12 晚 · 3 次）**：批次 `1329`/`1333`/`1335`；每次"取 `randCode` → 提交 → 登记 → **恢复** Redis 三类标记 / 预热库存 / DB 三值 → 复核"⇒ `seckill_ok=1`、`seckill_verify_bad=0`；**独立复核**（现状 vs 定点快照逐字段）全部回位。⚠️ 过程中抓到 **#69**（秒杀给**错误商品**加销量）→ 脚本已改成"**全量 sales 快照 + 按实际变化回补 + 纳入校验**"，三次都当场回补 ✅

⑤ ✅ **已完成（2026-09-12 晚）**：`--clean` 已补 **Redis 按登记坐标 `sku:user` 精确删**（三类键；dry-run 打印将删清单）—— 实测注入合成键 → **精确命中并删除**（未用 pattern 扫描）；**禁止 `--scan --pattern` 全删**的口径写进脚本注释

⑥ ✅ **已完成（2026-09-12 晚）**：`--baseline` 在造数**前**记基线（**11** 张表行数 + `stock`/`sales` 合计）· `--compare-baseline --batch X` 清理**后**比对（有差异退出码 1）—— 实测 **"基线 → 造数 → 秒杀 → 恢复 → 清理 → 比对"闭环 0 差异**；**#69 就是靠它抓出来的**
...
### 🗓️ 今晚执行清单（2026-09-12 晚 · 目标：#67 的 ④⑤⑥ + #68 兜底）—— ✅ **当晚已全部执行完毕**（④⑤⑥ 见上；**#68** 已关闭：全量 dump 已执行（`cs_mall_20260912_1452.sql.gz`，319KB / 6 库）+ `--require-dump` 闸门 + 自动登记 `dump_file`）
...
| **2（#67④）** | **秒杀真跑**（新机） | ⚠️ **必须换新前缀 + 新号段**（复用 `testsim0912`/`1390100` 会撞预检）；🔴 **且必须带 `--require-dump <快照文件名>`**（#68 已强制：不提供即拒绝开跑，见 `deploy/scripts/sim/README.md` §4.11）：<br>`SIM_DB_PASSWORD=… SIM_REDIS_PASSWORD=… SIM_USER_PREFIX=testsimsek SIM_PHONE_PREFIX=1391111 python3 /tmp/sim/simulate_data.py --days 1 --per-day 0 --users 5 --with-seckill --seckill-count 1`<br>（`--per-day 0` = 只建用户、**不跑漏斗**，让秒杀成为唯一变量） | 脚本**自动恢复**（Redis 三锁 + 预热库存 + DB 三值）→ 期望输出 `秒杀后恢复 1 次 / 校验未通过 0 次`；🔴 **绝不要加 `--no-seckill-restore`**；失败排查顺序：`--seckill-restore-wait`（默认 1.5s，MQ 异步回写要时间）→ 再看 **#68 的 dump** |
