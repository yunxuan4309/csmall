...
④ 🟡 **`--with-seckill` 脚本已就绪、只差真跑**（2026-09-12 实现：列表取 `randCode` → 提交 → 登记 → **每次动作后恢复** Redis 三类标记 / 预热库存 / DB 三值 → 复核；`--no-seckill-restore` 可关）

⑤ 🟡 **部分完成**：**秒杀后恢复**路径已按 id 精确删（`reseckill`/`ordered`/`orderLock` + 预热库存键）；**`--clean` 批次清理里的 Redis 部分仍未实现**（按登记 user id 精确拼 key；**禁止 pattern 全删**）
