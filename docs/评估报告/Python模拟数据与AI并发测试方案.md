...
> **状态**：✅ **规范设计已定稿 + 实施前复核完成，待实施**（对应 TODO **#48**）
> **修订链**：初稿（08-26）→ 规范设计定稿（09-10，补数据隔离层）→ **实施前复核（09-11，修正 mock 注入变量名 / Agent 工具轮 / 频控 / 单线程 mock 等 8 项 + 1 项澄清 + 决策"生产临时放开限流 + 双链路各压一遍"）**
...
```
【准备】
 0. 新机建 venv + requirements（requests/pymysql 钉版本）；DDL 与快照仍走老机 docker exec
 1. 写脚本骨架（含 fail-fast 预检 + dry-run 清理预览）+ 影子登记表 DDL + mock LLM v2（ThreadingHTTPServer + tool_calls）
 2. 在新机建 cs_mall_sim + 两张登记表；部署 mock LLM（内网 9999）；
    ★ 先单独压 mock 拿 P50/P99/吞吐 —— 证明 mock 不是瓶颈
 3. 压测前必查：compose 已透传 AI_API_BASE_URL + 容器内 base-url 已是 mock 地址（§〇.1 D1）
【第一层：造数】
 4. 确认基线干净（test_sim_% = 0）→ mysqldump 全量快照（实测仅 ~207M，成本极低）→ 记录基线快照
 5. 慢节奏造数（1~2 天，每天 1000 行为）→ 检查数据沉淀 + 登记表行数对得上
 6. dry-run 清理预览 → 演练一次"清理 → 比对照基线"
【第二层：AI 并发】
 7. 备份 Nacos 规则（落盘 + md5）→ 热改 ai-chat 阈值 → 构造 N 个模拟用户 token（N ≥ 120）
 8. 阶梯 20 → 50 → 100：先 AI_AGENT_ENABLED=false（流水线单轮），再 true（Agent 双轮）
    每档盯 docker stats / free -h；命中中止阈值（available<1.5G / 5xx+429>1% / p99>10s）立即停
 9. 测完：Nacos 改回 + GET 复核 → 移除 override → recreate → 验证 base-url 已回真实地址
【收尾】
10. 结果回填：sim_batch.note 或独立"压测结果表"（指标 → 数字 → 结论）
11. 决定：保留"演示数据" / 或整库还原到快照（复用 #47 独立容器先验证备份）
12. 面试演示素材：模拟数据统计 + AI 并发/闸门降级结果 + 限流 429 验证
    + ★ Agent（双轮）vs 固定流水线（单轮）的承载对比
```

> ~~初稿的"步骤 2：服务器内存优化 R7，找 ecs-user 执行"~~ —— **R7 已于 2026-09-07 执行完毕，该步删除**。
> **改动说明（2026-09-11）**：新增第 0 步（venv）、第 3 步（mock 地址必查）、第 2 步的"mock 自测"、第 7 步（Nacos 备份 + 多用户 token）、第 8 步的双链路与中止阈值、第 10 步（结果回填）、第 12 步的 Agent 对比 —— 逐条对应 §〇.1 的 D1~D9。
