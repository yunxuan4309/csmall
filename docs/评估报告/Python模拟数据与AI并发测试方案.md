...
> **状态**：🟢 **第一层「造数」+ 数据标识 + 可观测展示工具链已完成并归档**（2026-09-11）—— 明细 → [[TODO已完成]] **§十九**；原理与 8 处踩坑 → [[TODO第三批实现与原理-1]] **§十三**（对应原 TODO **#48**，**已归档**）
> 🔴 **剩余部分见 [[TODO文件]]#67**（#48 拆分而来）：① 录像 ② 正式造数（每天 1000 行为）③ **第二层「AI 并发压测」—— mock LLM / Nacos 规则放开-恢复 / Agent 双链路对比，完全未开始** ④ `--with-seckill` ⑤ Redis 精确清理 ⑥ `sim_baseline` 基线比对
> 🔴 **接续入口**：**§〇.2「当前进度与交接」**（决策 · 环境事实 · C-0~C-12 完成清单）+ **§6.4.1 录像 Runbook** + **§5.1 压测实测**；**剩余 6 项以 [[TODO文件]]#67 为准**
...
> **关联**：[[TODO文件]]**#48（第一层，已归档）** / **#67（= #48 剩余部分，本方案未完成部分全在此）**、[[TODO已完成]]#29（cron 备份）/ #47（恢复演练）、[[服务器内存优化方案]]（R7 已完成）、[[运维手册--密钥密码轮换]]、[[阿里云ECS服务器情况]]
> 🧩 **已提炼（2026-09-11）**：本文件的「§2.2 数据隔离 + §2.2.9 数据标识/回填矩阵 + §〇.1 复核 D1~D9 + §〇.2 §F 复核方法 + §G 校准实测 G1~G10」已提炼为「一类问题」文档 → **[[问题解决--生产造数的数据隔离与复核方法]]**（**原文件保留不动**，仅互链；本篇仍是方案正文 + 执行剧本 + 实测数据）
...
| ⑧ | **未覆盖** 2026-09-09 补充需求：~100 req/s + 每日 12 点高峰 + cron | TODO #67（原 #48） | 新增 §3.4 |
...
> | **② 可观测展示** | 第 5 步 `load_test.py` ✅ **完成（还超额：两档 + 自检 + 实测标定）**；**第 6/7 步「录像」⏳ 未做**（脚本 / §6.4.1 Runbook / [[演示录像操作手册]] 均已就绪） |
> | **③ 扩商品 20→60**（第 8 步） | ❌ 未做（可选；属 [[商品与秒杀扩容方案]] #63/#64） |
> | **④ 正式造数** | **第 9 步 ❌ 未做**（至今只跑过 **50 次动作的校准**，非"每天 1000 行为"）；第 10 步清理演练 ✅ |
> | **⑤ AI 并发压测**（第 11~15 步） | 🔴 **整层未开始** —— `mock_llm.py` **未写** · compose `AI_API_BASE_URL` 透传**未加** · Nacos 规则**未放开** · **Agent 双链路未压** |
> | **收尾** | 第 16 步 ⚠️ 部分（造数批次有 `note`；**压测结果未回填**）；第 17 步 ✅ 已决定**保留演示数据** |
...
**关联文档**：[[TODO文件]]**#48（第一层，已归档 → [[TODO已完成]] §十九）** / **#67（= #48 剩余部分）** / #3（场次维度购买标记）/ #31（向量检索）、[[TODO已完成]]#29（cron 备份）/ #47（恢复演练）、[[AI导购Agent升级方案]]（#32，Agent 可消费本方案造的丰富商品数据）、[[服务器内存优化方案]]（R7，已归档）、[[阿里云ECS服务器情况]]、[[TODO第三批实现与原理]]（两台服务器与内网拓扑）、**[[演示录像操作手册]]（#67 之"录像"的操作手册）**
