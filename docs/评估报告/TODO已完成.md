...
- **操作手册**：[[演示录像操作手册]]（**归属 #67**）
- **⏳ 剩余（[[TODO文件]]#67）**：① **录像** ② **正式造数**（每天 1000 行为）③ ~~**第二层 AI 并发压测**~~ ✅ **2026-09-12 首轮完成**（mock LLM + Nacos 规则放开/恢复 + Agent 双链路对比 → [[AI并发测试方案]] §十 · [[TODO第三批实现与原理-2]]）④ `--with-seckill` ⑤ **Redis 精确清理**（当前只打印告警）⑥ `sim_baseline` 基线比对
