...
> **状态**：🟡 **进行中（2026-09-12）** —— ✅ `mock_llm.py` 已写并**自测 14/14** + mock 自压基准（0 失败 / 峰值并发 150 / 306 req/s，证明 mock 不是瓶颈）· ✅ compose 已加 `AI_API_BASE_URL` 透传（D1）+ mock override · ✅ Nacos `ai-chat` 已热改 **5→200** · ✅ 800 个 AI 专用用户池已建（`testsimai*`，独立批次可整批清理）· 🔴 **阶梯压测被 #62 阻断**（`/ai/chat/stream` 并发下 **~50% 请求 HTTP 500**，根因 = ASYNC 二次派发被授权规则拒绝）→ **先修 #62，再压两条链路**
