...
| 📌 **未来计划** | **#48** · **#49** | Python 模拟数据 + AI 并发测试（**✅ 规范设计已定稿 2026-09-10**：影子登记表 + 快照回滚 + SSE mock + 内网压测）· 商品集群（验证 Dubbo 层负载均衡） |
| ✅ **本次已搬入文末** | **#32** · #44 · #42 · #9 · #6 · 第三批执行清单 · B/C 归档记录 · 10 行完成表格行 | 见文末「📦 已完成条目归档区」；**#32（AI 导购 Agent 升级）于 2026-09-11 全部完成归档** |
...
> **范围**：`docs/评估报告/` **现存 18 个**（见下方 📍 A）+ `docs/归档/` **本次移入 16 个**（📦 2026-09-10 已移至文末「📦 已完成条目归档区 · 四」，同处还有 ✅ 状态头修正 7 个）。其他目录（`问题解决/`、`运维/`、`归档/`、`面试准备/`）见 [[项目上下文文档]] 附录 B。

### 📍 A. `docs/评估报告/` 现存 18 个

> ⚠️ **计数口径**：本目录 `ls` 会显示 **19 个 `.md`** = **正式文档 18 个** + **1 个 `*-副本.md`**（`TODO第三批实现与原理-副本.md`，状态纠偏用的**工作副本，用户保留备用**）。
> 已删除的是第一/二批的副本（`TODO第一批/第二批实现与原理-副本.md`，2026-09-10 切割完成后按用户要求删除）。已在 `.gitignore` 加 `*-副本.md`。
> **做"定期交叉核对"时请排除 `*-副本.md`**，否则会误判数量（本表口径 = 18）。
...
| 文档 | 对应条目 | 状态说明 |
|---|---|---|
| `Python模拟数据与AI并发测试方案` | **#48** | ✅ **规范设计定稿（2026-09-10）**：数据隔离（影子登记表 + 快照回滚）、清理算法（逆序/分批/幂等/dry-run）、SSE mock、内网压测（~100 req/s + 12 点高峰） |
