...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-08（**第二批已全量部署服务器**：#33/#8/#36/#23/#14 P0+P1 上线完成，含 Flyway V6 + ES 索引清理；部署中修复 mall-ai AI 重排 reasoning 模型 bug。当前未完成：#13/#29/#5/#2+#34、#14 余 P2）
...
### 🔥 第二批：正确性 + 面试/演示价值（2026-09-08：代码批完成 + ✅ 已部署服务器）
...
> **部署状态（2026-09-08）**：✅ **第二批已全量部署上线**——11 个微服务 jar 全量重建（common/pojo 连带）+ Flyway V6 + 前端 dist + ES 索引清理全部完成，21 容器 Up、Nacos 全注册。**部署中额外发现并修复 mall-ai 既有 bug**：AI 重排偶发降级/超时，真因 = reasoning 模型过度思考致 content 截断（踩坑记录见 [[TODO第二批实现与原理]] §5.4.5 边界表 #21），解法 = JSON 任务用 `deepseek-chat`、SSE 对话保留 `v4-flash`。剩余待做：#13/#29/#5/#2+#34。
