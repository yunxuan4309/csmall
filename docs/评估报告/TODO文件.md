...
| 🆕 **已决策待实施** | **#32** | **AI 导购 Agent（Function Calling + ReAct，P0 → P1）** —— 前置 **#58 已于 2026-09-10 完成并部署**（模型契约 + 模型配置可配化 + 双 bean 隐患修复）。**P0 代码已完成**（`09d22b1`，离线测试 26 项全绿、开关默认关）；**待执行 P0 部署**（两阶段：先验零回归再开开关）。📖 选型过程与实测见 [[TODO第三批实现与原理-1]] |
...
| **P0 部署** | ⏳ **待执行**（指令已备好）| 两阶段 runbook：**A 只换 jar 验零回归 → B 开 `AI_AGENT_ENABLED=true` 验 Agent**；回滚三级（① 关开关 45s / ② 换回 jar+镜像 / ③ 连 compose）|
