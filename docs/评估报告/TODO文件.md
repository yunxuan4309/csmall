...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-08（**第二批代码批全部完成**：#33 A2 落地+本地验证通过、#8/#36/#23/#14 P0+P1 均已完成——全部**待部署服务器**；本文件保留未完成/部分完成/暂缓项。已完成明细迁 [[TODO已完成]]，原理与实现见 [[TODO第二批实现与原理]]。当前未完成：#13/#29/#5/#2+#34、#14 余 P2）
...
### 🔥 第二批：正确性 + 面试/演示价值（2026-09-08：代码批全部完成，待部署服务器）

| 顺序 | 编号 | 事项 | 状态（2026-09-08） |
|------|------|------|-----------|
| 1 | **#33** | 双索引数据不一致 | ✅ **A2 方案落地完成 + 本地验证通过**（统一单一索引 + mall-search 只读降级层 + 同步模型补全 + 前端 fallback）。本地实测：AI 主链路正常（意图/扩展/重排全通）、停 mall-ai 前端秒级降级到普通搜索。**待部署** |
| 2 | **#8** ✅ | AI 预算按北京时间结算 | ✅ 已完成（2026-09-07，TokenBudgetService 时区）→ 见 [[TODO已完成]]；**待部署** |
| 3 | **#36** ✅ | DLX 死信 + OrderQueueConsumer requeue 修复 | ✅ 已完成（x-death 限次重试 + 订单队列 DLX + OrderDlxConsumer）；秒杀消费者静默丢弃见 #14 → 见 [[TODO已完成]]；**待部署** |
| 4 | **#13** | Nacos 开启认证 | ⏳ 未做（内网失陷可注册假服务） |
| 5 | **#14** 🟡 | Redis 主从切换防数据（五层） | ✅ P0 三层 + P1 对账已完成 → 见 [[TODO已完成]] §六；只余 P2 配置层（随 #9）；**P0/P1 待部署** |
| 6 | **#29** | 数据库定期备份 | ⏳ 未做（运维底线） |
| 7 | **#23** ✅ | 漏触发接口补 @Validated | ✅ 已完成（DTO 补规则 + @Validated + 全局异常补全）→ 见 [[TODO已完成]]；**待部署** |
| 8 | **#5** | Sentinel 能力补齐 | ⏳ 未做（3 接口规则空转 + 热点限流） |
| 9 | **#2 + #34** | AI 接口限流 + 并发闸门 | ⏳ 未做（AI 慢请求占线程） |

> **部署状态**：第二批代码批（#33/#8/#36/#23/#14 P0+P1）已在本地完成并编译/自检通过，**服务器仍运行 2026-08-04 旧 jar**——需一次维护窗口全量构建部署（涉及 mall-search/mall-ai/mall-product/mall-order/mall-seckill/mall-ams/mall-ums/mall-common/mall-pojo + Flyway V6）。剩余待做：#13/#29/#5/#2+#34。
...
### 33. 【搜索】双索引数据不一致（mall-search 与 mall-ai 各维护一个 ES 索引）✅ 已完成（2026-09-08，代码+本地验证，待部署）

> ✅ **2026-09-08 完成（方案 A2）**：统一单一索引 + mall-search 改造为**只读普通搜索降级层** + 同步模型补全 + 前端 fallback。本地实测：AI 主链路（意图/扩展/重排）全通；`docker stop mall-ai` 后前端秒级降级到普通搜索。明细见 [[TODO第二批实现与原理]] §五、[[搜索双索引统一与一致性评估]]。
