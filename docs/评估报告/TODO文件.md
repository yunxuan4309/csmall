...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-10（**新增「📚 方案文档索引」**——反查"某方案文档有没有被 TODO 跟踪"，并立下"新增文档必须登记"的纪律；修正路线图过时表述；补齐 4 处方案文档链接；新登记 **#57**（Schema 漂移服务器待核实）。此前 09-09：跨机集群阶段 0~5 全部完成并归档 [[TODO已完成]] §十三；#14-P2 运行时已开启）
...
## 📚 方案文档索引（2026-09-10 建立 · `docs/评估报告/` 全量）

> **用途**：反查「某个方案文档有没有被 TODO 跟踪」。
> **⚠️ 纪律**：**新增任何方案 / 评估文档时，必须在本表登记** —— 未登记 = 漏跟踪；同时该条 TODO 里要 `[[链接]]` 对应方案文档（双向索引）。
> **状态口径**：🟡 待实施 / 进行中 · ✅ 已完成（明细见 [[TODO已完成]]）· ⏸️ 暂缓 / 仅评估 · 📄 存档或索引类。
> **范围**：本表只覆盖 `docs/评估报告/`（方案 / 评估 / 执行清单类，共 31 个文件）。其他目录的文档（`问题解决/`、`运维/`、`归档/`、`面试准备/`）见 [[项目上下文文档]] 附录 B「文档索引」。

### 🟡 待实施 / 进行中

| 文档 | 对应条目 | 状态说明 |
|---|---|---|
| `Redis主从切换防数据问题方案` | **#14-P2** | 主库已开 `min-replicas-to-write 1`；**2 台机器的固有局限见 [[TODO第三批实现与原理]] §6.9** |
| `Sentinel能力补充计划` | **#5-P1** | P0 已完成（第二批 #5-P0）；P1 热点参数限流待做 |
| `Python模拟数据与AI并发测试方案` | **#48** | 方案定稿（真实并发 ~100 req/s + 每日 12 点高峰窗口） |
| `连接池统一HikariCP方案` | **#R5** | 6 服务 Druid→HikariCP；需全量回归 + 重启窗口 |
| `认证安全企业级升级方案` | **#17** | 待实施（评估稿已定稿） |
| `TraceId链路日志规范方案` | **#16** | 待实施（SkyWalking logback 集成） |
| `数据库Schema漂移审计` | **#57**（2026-09-10 新登记） | 本地已修复；**服务器是否已执行 ALTER 待核实** |
| `集群化与配置中心迁移方案` | **#4** | 秒杀已跨机双实例 ✅；**配置中心未迁 Nacos** |

### ✅ 已完成（明细见 [[TODO已完成]]）

| 文档 | 对应条目 | 归档位置 |
|---|---|---|
| `服务器内存优化方案` | R7（第一批） | [[TODO已完成]] §一 / 正文 R7 |
| `Redis配置加固与哨兵模式方案` | R1~R4（第一批）+ #9（第三批） | [[TODO已完成]] §二 + §十三 |
| `JVM调优方案` | JVM 三档调优（G1GC / 分堆 / Metaspace） | [[TODO已完成]] §五 前 |
| `秒杀对账任务实现方案` | #14-P1（第二批） | [[TODO已完成]] §六 |
| `搜索双索引统一与一致性评估` | #33（第二批） | [[TODO已完成]] §九 |
| `第二批部署执行清单-2026-09-08` | 第二批全量（正确性 + 演示 9 项） | [[TODO已完成]] §三 / §六~§十二 |
| `Sentinel部署执行清单-2026-09-08` | #5-P0（第二批） | [[TODO已完成]] §七 |
| `AI限流与并发闸门-部署执行清单-2026-09-08` | #2 + #34（第二批） | [[TODO已完成]] §八 |
| `Nacos认证-部署执行清单-2026-09-08` | #13（第二批） | [[TODO已完成]] §十一 |
| `本地双实例锁验证报告-2026-09-09` | #4-阶段0（第三批） | [[TODO第三批实现与原理]] §四 |
| `跨机集群方案-讨论纪要-2026-09-09` | 跨机集群（第三批 · 方案讨论） | [[TODO第三批实现与原理]] §五 |
| `跨机集群实施执行清单-2026-09-09` | 跨机集群（第三批 · 命令级） | [[TODO已完成]] §十三；实测见其 §5.5 / §6.5 / §7.6~§7.9 |

### ⏸️ 暂缓 / 仅评估 / 存档

| 文档 | 对应条目 | 状态 |
|---|---|---|
| `AI导购Agent升级方案` | **#32** | ⏸️ 仅计划不执行（面试/学习价值高，业务收益为零） |
| `Kubernetes与ZooKeeper评估` | **#15** | ⏸️ k3s 方案备用，暂不部署 |
| `Redis集群适配与配置管理评估` | **#12** | ⏸️ 仅评估不实施（数据量小，HA 已由哨兵覆盖） |
| `服务器巡检与待修复问题清单-2026-08-04` | 「⏸️ 已确认问题」章节 | 部分已修，余项演示项目暂缓 |
| `企业级生产评估报告-2026-08-15` | —（非计划，纯基线报告） | 📄 存档，供后续整改参考 |
...
| `TODO第一批实现与原理.md`、`TODO第二批实现与原理.md`、`TODO第三批实现与原理.md` | 原理 + 踩坑 + 疑惑点 + 面试话术（与本文件配套的三个深挖文件） |
| `待实施计划总览.md` | ⚠️ 2026-08 的历史快照，**已由本表取代**（其文件头已加时效说明） |

---

## 🏷️ 文档分类标记（2026-09-10 评估 · **待执行，暂未移动任何文件**）

> **本次只标记「类型 / 未来去向」，不移动文件。** 下一步整理时按本表执行。
> **标记口径**：
> - 🔴 **即将归档** = 过期的 / 已执行完毕的（计划已做完、评估已定论、快照已被取代）
> - 🟢 **问题解决文档** = 讲述**实现过程、方法、遇到的问题**（知识 / 经验记录）
> - ⚪ **其他文档** = 仍待办的计划 / 活跃状态文件 / 参考类
>
> **建议去向**：🔴 → `docs/归档/`　🟢 → `docs/问题解决/`　⚪ → **留在 `docs/评估报告/`**
...
| `跨机集群方案-讨论纪要-2026-09-09` | 方案已拍板且**全部实施完毕**（⚠️ 含答疑与演练剧本，可保留指针） |
| `服务器内存优化方案` | ✅ 已执行完毕（R7：21 容器 mem_limit + Nacos 降堆 + Swap 2G） |
| `JVM调优方案` | ✅ 已执行完成（2026-08-22 确认全部生效） |
| `Redis配置加固与哨兵模式方案` | ✅ 加固（R1~R4 09-07）+ 哨兵（#9 09-09）均已完成 |
| `Redis主从切换防数据问题方案` | ✅ 已实施（#14-P2 已开启，2026-09-09） |
| `秒杀对账任务实现方案` | ✅ 代码实施 + 已随第二批部署（#14-P1） |
| `搜索双索引统一与一致性评估` | ✅ 已实施并部署（#33，归档 §九） |
| `服务器巡检与待修复问题清单-2026-08-04` | 08-04 巡检快照；余项已收口到本文件「⏸️ 已确认问题」章节 |
| `企业级生产评估报告-2026-08-15` | 一次性基线快照，整改项已被三批 TODO 覆盖 |
| `Redis集群适配与配置管理评估` | ✅ 评估完成，结论 = **暂不实施**（仅评估） |
| `Kubernetes与ZooKeeper评估` | ✅ 评估完成，结论 = **不部署**（k3s 仅备用方案） |
| `AI导购Agent升级方案` | 评估定稿且**决定不执行**（#32：仅面试/演示素材） |
| `待实施计划总览` | ⚠️ 2026-08 历史快照，已被本文件顶部索引表取代 |

### 🟢 问题解决文档（5 个）

| 文档 | 判定依据 |
|---|---|
| `TODO第一批实现与原理` | 第一批实现过程 + 踩坑 + 面试话术 |
| `TODO第二批实现与原理` | 第二批实现过程 + 踩坑 + 话术 |
...
|---|---|
| `TODO文件` | ⭐ **状态源**（活跃） |
| `TODO已完成` | 已完成归档索引（活跃维护） |
| `Python模拟数据与AI并发测试方案` | 方案定稿，**待实施**（#48） |
| `Sentinel能力补充计划` | P0 已完成，**P1 待做**（#5-P1） |
| `TraceId链路日志规范方案` | 评估 + 计划，**待实施**（#16） |
| `认证安全企业级升级方案` | 评估 + 计划，**待实施**（#17） |
| `连接池统一HikariCP方案` | 方案定稿，**暂缓执行**（#R5） |
| `集群化与配置中心迁移方案` | 集群化部分已完成（跨机双实例 ✅），**配置中心迁移未做**（#4） |

### ⚠️ 顺带发现：7 个文档的「状态头」已过期（写"待执行"，实际已完成）

> 这是**另一种文档漂移**——正文/归档都已完成，但文件头部的 `> **状态**` 还停在"待执行"，容易误导后续阅读。**建议在归档/迁移时一并修正**。

| 文档 | 头部写的 | 实际状态 |
|---|---|---|
| `TODO第三批实现与原理` | 🟡 逐项实施中 / 阶段 2a/2b 改动已备好 | **阶段 0~5 全部完成**（09-09） |
| `Nacos认证-部署执行清单-2026-09-08` | 📋 待服务器执行 | **已执行并部署**（#13） |
| `Sentinel部署执行清单-2026-09-08` | 📋 待服务器执行 | **已执行**（#5-P0） |
| `Redis配置加固与哨兵模式方案` | 哨兵模式部分仍未执行 | **哨兵已完成**（#9，09-09） |
| `Redis主从切换防数据问题方案` | 📋 已定稿待实施 | **已实施**（#14-P2） |
| `搜索双索引统一与一致性评估` | 🟡 待服务器部署 | **09-08 已部署**（#33） |
| `秒杀对账任务实现方案` | 🔧 需重启 seckill 加载新类 | **已随第二批部署**（#14-P1） |
...
> **进行中**：🟢 第三批（企业级演进 + 学习）。**跨机集群主线（#4 秒杀双实例 + #9 Redis 主从哨兵 + #14-P2）阶段 0~5 已全部完成**（2026-09-09）→ 归档 [[TODO已完成]] §十三、原理见 [[TODO第三批实现与原理]] §五/§六。**剩余事项**：#5-P1 热点参数限流、#14-P2 持久化（待 sudo）、#40/#45/#48/#49、安全类 #51/#53/#54/#55/#56/#57。时间紧迫时**按下方「🧭 推荐执行路线」取舍**：P1=#5-P1（面试主菜）→ P2=#14-P2/#40 → P3=其余只讲认知。
