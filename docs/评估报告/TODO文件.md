...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-11（**三批改动**：① **结构调整 · 仅搬位置、内容零删改**：把正文中**已完成**的条目、表格行、文档归档记录共 **12 处**统一移到文末 **「📦 已完成条目归档区」**，并在顶部新增 **「📌 待办速览」**；② **#32 AI 导购 Agent 敲定**（用户拍板「做」，P0 → P1 分期，方案补 6 条实施前代码校正）；③ **#48 Python 模拟数据规范设计定稿**（补数据隔离层：影子登记表 + 快照回滚 + SSE mock + 内网压测）；④ **#58 复验通过并敲定**（7 格实测实验复现 400 规则 + 证实 `thinking` 开关；决策：方案 A + 新增 `chatJson` + 删 `compare-model`，与 #32 同窗口分两步）；⑤ **模型配置可配化并入 #58 Step 1**（占位符外置 + 能力档位层 → 改名只改 `.env` 不重编译）+ **新增 #60 Spring AI 引入评估**（结论：暂不引入，前置 = Boot 全站升级）。历史上最近一次实质更新为 2026-09-10：文档归档整理 + 状态纠偏 + 文档切割 + #44 key 轮换 + 新增 #58/#59）
...
| 🔴 **可用性风险** | **#51** · **#53** | restart 改 `unless-stopped`（老机 20 + 新机 5，需 recreate）· 网关重试 / 优雅下线兜底（需重启网关窗口） |
| 🔴 **安全** | **#55（P1）** · **#54** · **#56** | SSH 允许密码 + root 登录（两台）· RabbitMQ 仍 `guest/guest` · 公开仓库暴露 IP/拓扑（当前选择"接受"） |
| 🟠 **待决策（方案已出）** | **#31** | 生产开向量检索（🔴前置 **#59** 硅基流动充值） |
| 🆕 **已决策待实施**（同一维护窗口，分两步） | **#58** → **#32** | **#58 先做**：`deepseek-chat` → 现役 id + 显式 `thinking` 开关 —— 🔧 **代码已完成（2026-09-11，本地全绿，⏳ 待部署）**；**#32 第二步**：AI 导购 Agent（Function Calling + ReAct，P0 → P1）。📖 **选型过程与实测见 [[TODO第三批实现与原理-1]]** |
...
| 📌 **未来计划** | **#48** · **#49** | Python 模拟数据 + AI 并发测试（**✅ 规范设计已定稿 2026-09-11**：影子登记表 + 快照回滚 + SSE mock + 内网压测）· 商品集群（验证 Dubbo 层负载均衡） |
| ✅ **本次已搬入文末** | #44 · #42 · #9 · #6 · 第三批执行清单 · B/C 归档记录 · 10 行完成表格行 | 见文末「📦 已完成条目归档区」 |
...
> **范围**：`docs/评估报告/` **现存 17 个**（见下方 📍 A）+ `docs/归档/` **本次移入 16 个**（📦 2026-09-11 已移至文末「📦 已完成条目归档区 · 四」，同处还有 ✅ 状态头修正 7 个）。其他目录（`问题解决/`、`运维/`、`归档/`、`面试准备/`）见 [[项目上下文文档]] 附录 B。

### 📍 A. `docs/评估报告/` 现存 17 个

> ⚠️ **计数口径**：本目录 `ls` 会显示 **18 个 `.md`** = **正式文档 17 个** + **1 个 `*-副本.md`**（`TODO第三批实现与原理-副本.md`，状态纠偏用的**工作副本，用户保留备用**）。
> 已删除的是第一/二批的副本（`TODO第一批/第二批实现与原理-副本.md`，2026-09-10 切割完成后按用户要求删除）。已在 `.gitignore` 加 `*-副本.md`。
> **做"定期交叉核对"时请排除 `*-副本.md`**，否则会误判数量（本表口径 = 17）。
> 🆕 **2026-09-11 新增**：`TODO第三批实现与原理-1.md`（第三批**补册 1**：Agent + 模拟数据 + 技术选型过程）

#### 🟡 待实施 / 暂缓（7）

| 文档 | 对应条目 | 状态说明 |
|---|---|---|
| `Python模拟数据与AI并发测试方案` | **#48** | ✅ **规范设计定稿（2026-09-11）**：数据隔离（影子登记表 + 快照回滚）、清理算法（逆序/分批/幂等/dry-run）、SSE mock、内网压测（~100 req/s + 12 点高峰） |
...
| `AI模型名停用风险与thinking参数改造方案` | **#58** | ✅ **已决策实施（2026-09-11 复验通过）**：方案 A（现役 id + `thinking` 开关）+ 新增 `chatJson` + 删 `compare-model`；§3.1 含 **7 格实测实验**（复现 400 规则 / 证实开关生效 / 证实 tools 可行 / 发现 tools 与 json mode 互斥） |
...
| 文档 | 说明 |
|---|---|
| `AI导购Agent升级方案` | ✅ **2026-09-11 用户拍板「做」**：P0（最小 Function Calling）→ P1（完整单 Agent）分期；方案已敲定（含 **6 条实施前代码校正** + 文件级执行清单 + 验证 + 回滚）。对应 TODO **#32** |
...
| `TODO第三批实现与原理-1` | 🆕 **补册 1（2026-09-11 新建）**：**#32 Agent + #48 模拟数据** 的设计与**技术选型过程**（Spring AI 评估 / Boot 升级影响面 / 模型配置可配化）+ **10 格实测实验**。因正册 [[TODO第三批实现与原理]] 已 946 行过长，按用户要求另开一册，**正册不动、两册互链** |
...
| `问题解决--服务注册与网关路由` | 一个服务在注册中心**有多个身份 → 路由打错端口** | 二批 #6 |
...
| ~~决定是否做 AI 导购 Agent 升级~~ | `AI导购Agent升级方案`（**#32**） | ✅ **已决定：做（2026-09-11）** → P0 → P1 分期实施；详见该文档「〇、决策记录 + 实施前代码校正」 |
...
| **#48** | **Python 模拟数据 + AI 并发测试**（面试数据素材） | ✅ **规范设计定稿（2026-09-11，待实施）**：① 慢节奏造"运营数据"（80/15/5 漏斗）② mock LLM 测 AI 服务端并发承载（不调真实 API）+ **真实并发 ~100 req/s + 每日 12 点高峰窗口（cron）**。<br>🔴 **本次补上「数据隔离层」**——初稿只有用户名前缀，实测判定**不够**：清理 SQL 顺序错误（先删用户 → 订单删不掉）、`pms_spu.sales` / 库存 / Redis 秒杀预热键**不可逆污染**、Redis 清理想按 pattern 全删会**误伤真实数据**、mock 未实现 SSE 格式（会收不到 chunk）。<br>**定稿方案 = 影子登记表**（`cs_mall_sim.sim_batch` / `sim_entity`，清理唯一依据；逆序 + 分批 + 幂等 + dry-run）**+ 快照回滚兜底**（不可逆字段不硬算）**+ Redis 按 id 精确删**；mock 部署**新机内网**（容器内 `127.0.0.1` 不可达）且**必须支持 SSE 分片**；压测走内网并**避开 Sentinel 限流接口**。<br>⚠️ 脚本须在**新机/内网**跑（老机 5Mbps 自压自伤）。方案：[[Python模拟数据与AI并发测试方案]]（2026-09-11 规范设计版）；**隔离设计的选型过程与实测证据见 [[TODO第三批实现与原理-1]] §四** |
...
| **#58** | **AI 依赖已公告停用的模型名 `deepseek-chat`（AI 可用性隐患）** | 🟠 **P2（2026-09-10 复查 AI 模型配置时发现）**：生产 `mall-ai` 实际生效 `chat-model: deepseek-chat`（**实测解包生产 jar 确认**，来源 commit `c0d7209` / 2026-09-08）。但官方更新日志（2026-04-24）已公告 **`deepseek-chat` / `deepseek-reasoner` 于 2026-07-24 停止使用** —— 实测至今仍 HTTP 200（兼容层未撤），**属"靠别人宽容在跑"的技术债，随时可能失效**（一旦撤销 → 意图提取/重排/偏好提取/RAG 全线降级）。<br>**根因**：当初为绕开"`deepseek-v4-flash` 是思考模型、对 JSON 小任务思考到吃满 `max_tokens` 把 `content` 挤空"而**改用另一个模型名**（[[TODO第二批实现与原理]] §5.4.5 #21）—— 方向对但手段过时。<br>**✅ 正解（官方机制）**：DeepSeek 提供的是「**同一模型 + `thinking` 开关**」：`{"thinking":{"type":"disabled"}}` 从根上关掉思考（不再产出 `reasoning_content`），而非换模型名；JSON 任务 `disabled`、深度对话 `enabled` + `reasoning_effort`。<br>**顺带发现**：① `deepseek-chat` 与 `deepseek-v4-flash` 实测**底层已指向同一个 `deepseek-flash`**（差异只在思考模式）；② SSE 分支的 `temperature` 在思考模式下**是死参数**（官方明确不生效）；③ `compare-model` 配置项**零引用**（死配置）；④ `GET /models` **不是权威清单**（不列 `deepseek-chat` 但该名字仍可调）。<br>📄 **完整方案（问题证据 / 官方参数 / 8 个调用点分类 / 文件级改动清单 / 3 个 `doChat` 选项 / 风险回滚 / 验证清单 / 面试话术 / 备选方案）见 [[AI模型名停用风险与thinking参数改造方案]]** 🔧 **Step 1 代码已完成（2026-09-11）· ⏳ 待部署** —— 本地：全反应堆 13 模块编译 + 121.5 MB jar 打包 + **7 项测试全绿**；实现「**档位层 + 任务层**」模型可配化，**9 项硬编码全部清零**（`src/main` 已无代码级模型名）；**关键修正**：查询扩展输出纯文本，须单列任务类型（否则带 `response_format` 会 400）。实施记录见方案文档 **§十一**。以下为复验与决策依据：**10 格实测实验复现了 400 规则**（思考模式 + tools 不带 `reasoning_content` → HTTP 400）并**证实 `thinking: disabled` 生效**（无 `reasoning_content`）；用户确认**按推荐执行**（采纳方案 A、`doChat` 走新增 `chatJson`、**删除** `compare-model` 死配置），**与 #32 同维护窗口分两步**（先本项）。<br>🔴 **顺带 3 个新发现**：① `tools` 与 `response_format: json_object` **互斥**（工具轮不能设 json mode，否则模型只输出 JSON 不调工具）；② SSE 分支硬编码 `max_tokens:2000` 偏紧（思考吃满会把 content 挤空）→ 应改读配置；③ `DeepSeekAiClient.embed/embedBatch` 是死代码（无人调用，真调用必失败）。⚠️ **触发条件**：官方一旦真撤 `deepseek-chat`，本条立即从"优化"升级为"必须做"。 |
...
| **#60** | **Spring AI 引入评估（结论：暂不引入，前置 = Boot 全站升级）** | 🟡 **P3（2026-09-11 评估，源自"模型配置能否适应更名/下架"的架构提问）**：<br>**诉求**：模型配置要能适应**未来模型更名 / 下架**（改名不改代码、不重编译）。<br>**现状实测**：**半可配** —— Key / API 地址已占位符化 ✅；但**模型名是字面量**（`chat-model: deepseek-chat`）+ **2 处硬编码在代码里**（`ChatServiceImpl:316` / `:367`）；prod **未覆盖** `cooxiao.ai` 段 → 生产模型名来自 jar 内 yml → **换模型必须重编译**。<br>**🟢 决策（2026-09-11）：本次不引入 Spring AI**，改为"**手写 + 模型配置可配化**"落地（随 #58 Step 1，第 1+2 层：占位符外置 + 能力档位层 `models`/`tasks`，代码永不出现模型名）→ 改名 = 改 `.env` + 重建 `mall-ai` 一个容器。<br>🔴 **不引入的硬理由**：Spring AI 官方支持代际为 **Boot 3.4.x→1.0.x / 3.5.x→1.1.x / 4.x→2.x**，**Boot 3.3 及更早不在支持范围** → 本项目 **Boot 3.2.5 低于支持下限**，接入必须先升 Boot ≥3.4，连带 **Spring Cloud 2023.0.3→2024.0.x + SCA 2023.0.1.2→新一代**，**11 个服务 + 跨机集群全量回归** = **全站框架升级项目**（非 AI 功能项目）。<br>**未来口子**：第 2 层"档位 + 任务映射"是**框架无关抽象**，将来迁到 Spring AI `ChatClient`/`ChatOptions` 可直接映射，本次投入不浪费。Spring AI 保留为 #32 的 **P2（框架化）**，触发条件 = 有 Boot 升级窗口。<br>📄 评估详见 [[AI模型名停用风险与thinking参数改造方案]] §4.5 / §4.6 / **§4.7（升级 Boot 影响面实测：Boot 版本在根 pom → 13 个模块全体继承，"只影响 AI 模块"不成立；连带 Spring Cloud 2023.0.3→2024.0.x + SCA 同代际 + springdoc/MP/Dubbo 等全套迁移 + 两台机器 12 个服务回归）** |
...
**执行建议（时间紧张时 · 2026-09-11 更新，已剔除已完成项）**：
...
> 🔴 **2026-09-10 前置条件（P0 级阻断）**：开启前**必须先解决 [[TODO文件]] #59** —— 硅基流动账户**余额不足**，`POST /v1/embeddings` 实测返回 **402**（新 key 认证本身有效）。即：**代码开关一开，向量化立刻全失败**（`RagServiceImpl` 会走向量分支并抛错），所以本项**当前不具备开启条件**。
...
**开启评估（用户倾向：原以为免费所以值得开 → ⚠️ 2026-09-10 实测额度不足返 402，见 #59）**：
1. prod yml 改 `embedding-enabled: true` → 重启
2. 重建/补索引：EsIndexInitializer 需建含 dense_vector(1024) 的 mapping（当前索引无向量字段）
3. sync-auto-on-startup 全量向量化（20 条；⚠️ 原记"免费"，实测额度不足返 402，见 #59）
...
### 32. 【AI】AI 导购升级 Agent（Function Calling + ReAct）（**✅ 已决策实施 2026-09-11**，P0 → P1 分期）

> **2026-09-02 新增（源自 09 Q0/Q12 讨论）**：当前 AI 导购 = "LLM 解析意图 + 代码写死执行"（无工具调用/无循环/无动作决策权）；升级 Agent = 技术演示增强 + 面试素材（生产 0 调用，业务收益为零）。
>
> ✅ **2026-09-11 决策（用户拍板）：做** —— 按 **P0（最小 Function Calling）→ P1（完整单 Agent）** 分期推进，P2 框架化可选。方案已敲定，**含 6 条实施前代码校正**：① `mall-ai` **无任何数据库栈** → 原 P1「审计落 DB + Flyway」不可行，改为只落 Redis；② 消息类型须加宽为 `Map<String,Object>`；③ Agent 循环只放**非流式**路径（最后一轮复用现有 SSE）；④ `RagServiceImpl` 待复用的是**包私有**方法；⑤ 预算/闸门/限流**全部复用**既有设施；⑥ 与 **#58** 强耦合。
...
📄 **完整方案（现状/差距/场景工具集/9 条边界/分阶段到文件级/成本风险/面试话术/执行清单）见 [[AI导购Agent升级方案]]**；**技术选型过程（为何手写而非 Spring AI）+ 10 格实测实验 + 模拟数据设计见 [[TODO第三批实现与原理-1]]**
...
> **搬移记录**：2026-09-11 按用户要求搬移（已完成项 → 文末；新增顶部「📌 待办速览」）。搬移前版本见 git 历史。
