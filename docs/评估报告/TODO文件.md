...
| 🔴 **新发现（待决策 · 不夹带）** | **#64** · **#65** | **#64** 秒杀预热**两套 `spu_id` 语义冲突** → **4/12 个秒杀 SKU 从不预热**，靠凌晨对账写的永久 key 偶然兜住（**#63 已修复，见下行**）<br>🆕 **#65**（2026-09-11 由 #48 校准发现）**普通订单的库存扣减 MQ 链路整体失效** → `pms_sku.stock` **永不减少**（消费者/死信消费者都收不了被 JSON 转换器转成的 `ArrayList`）→ 既影响演示数据真实性，也意味着**下单无库存校验** |
