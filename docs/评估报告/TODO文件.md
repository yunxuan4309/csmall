...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-10（**五类改动**：① **首次文档归档整理完成** —— 16 个已执行完毕的文档移入 `docs/归档/`，索引表重构为「现存 **16** + 归档 16」，16 处相对路径引用同步修正，7 处过期状态头修正；② **第一批/第二批「实现与原理」状态纠偏** —— 逐条 SSH 实测 + 代码核实，修掉 Swap/端口/#29 备份/#13/#2+#34/#5.4.5「待部署」等过期表述；③ **新登记 #58**（AI 依赖已公告停用的 `deepseek-chat`）+ **#51 两机复核**（官方文档证实 `on-failure` 不扛 daemon 重启，老机 20/21、**新机 5/5** 不恢复）。同日另：新增「📚 方案文档索引」并确立"新增文档必须登记"纪律；新登记 #57；AI 导购 Agent（#32）状态澄清为"待决策"而非"不执行"、后明确为「暂时不评估，先放着」。④ **文档切割** —— 两批「实现与原理」的技术正文**切割为 `docs/问题解决/` 下 8 个「一类问题」聚合文档**（原文件改为**薄索引**：索引 + 执行记录 + 话术），并生成 #58 方案文档；④ **#44 Embedding Key 轮换收口**（旧 key 吊销 + 全仓库换新 + 服务器 `.env` + `mall-ai` recreate，实测通过）+ **新登记 #59**（硅基流动余额不足，embedding 402，待充值）；⑤ **#40 状态更正**（Step1 已就绪但 `/actuator/health` 被 SSO 拦截返回"假健康"，Step2 需先放行白名单）。此前 09-09：跨机集群阶段 0~5 全部完成并归档 [[TODO已完成]] §十三）
> **本文件定位**：**唯一状态源**（未完成 / 暂缓 / 评估 / 第三批事项）；已完成明细在 [[TODO已完成]]，原理/踩坑/面试话术在 [[TODO第三批实现与原理]] 等三个"实现与原理"文件
> **关联文档**: [[TODO已完成]]、三个「实现与原理」文件；**已归档的方案文档**见 [[TODO文件]] 顶部「📦 B. `docs/归档/` 本次移入 16 个」（含 [[服务器巡检与待修复问题清单-2026-08-04]]、[[Redis配置加固与哨兵模式方案]]、[[服务器内存优化方案]]、[[集群化与配置中心迁移方案]]、[[Sentinel能力补充计划]]、[[Redis主从切换防数据问题方案]]、[[TraceId链路日志规范方案]]、[[认证安全企业级升级方案]] 等）
...
> **状态口径**：🟡 待实施 / 暂缓 · 🟢 问题解决文档 · ⏸️ 待决策 · ✅ 已完成 · ⚪ 状态源 / 索引类 · 📦 已归档
> **范围**：`docs/评估报告/` **现存 16 个** + `docs/归档/` **本次移入 16 个**（2026-09-10）。其他目录（`问题解决/`、`运维/`、`归档/`、`面试准备/`）见 [[项目上下文文档]] 附录 B。

### 📍 A. `docs/评估报告/` 现存 16 个

> ⚠️ **计数口径**：本目录 `ls` 会显示 **17 个 `.md`** = **正式文档 16 个** + **1 个 `*-副本.md`**（`TODO第三批实现与原理-副本.md`，状态纠偏用的**工作副本，用户保留备用**）。
> 已删除的是第一/二批的副本（`TODO第一批/第二批实现与原理-副本.md`，2026-09-10 切割完成后按用户要求删除）。已在 `.gitignore` 加 `*-副本.md`。
> **做"定期交叉核对"时请排除 `*-副本.md`**，否则会误判数量（本表口径 = 16）。

#### 🟡 待实施 / 暂缓（7）

| 文档 | 对应条目 | 状态说明 |
|---|---|---|
| `Python模拟数据与AI并发测试方案` | **#48** | 方案定稿（真实并发 ~100 req/s + 每日 12 点高峰窗口） |
...
| `AI模型名停用风险与thinking参数改造方案` | **#58** | 🆕 2026-09-10 新增：方案已出，**待用户决策是否实施**（生产依赖已公告停用的 `deepseek-chat`）|
...
| 文档 | 说明 |
|---|---|
| `AI导购Agent升级方案` | ⏸️ **技术方案已出，⏸️ 用户 2026-09-10 明确「暂时不评估，先放着」** —— *尚未做、也尚未评估要不要做*（**不是「不做」**）。对应 TODO **#32** |
...
| 决定是否做 AI 导购 Agent 升级 | `AI导购Agent升级方案`（**#32**） | ⏸️ **用户 2026-09-10 明确：暂时不评估，先放着**（不删除、不归档，文件保留在 `评估报告/`；等用户主动提出再评估） |
...
### 32. 【AI】AI 导购升级 Agent（Function Calling + ReAct）（2026-09-02 出方案，**待决策**）

> **2026-09-02 新增（源自 09 Q0/Q12 讨论）**：当前 AI 导购 = "LLM 解析意图 + 代码写死执行"（无工具调用/无循环/无动作决策权）；升级 Agent = 技术演示增强 + 面试素材（生产 0 调用，业务收益为零）。
>
> ⏸️ **2026-09-10 状态澄清（用户）**：本项**尚未实施，也尚未评估要不要做** —— 技术方案已出，但「做不做」的决策仍在用户手上（**不是「不做」**）。**用户同日进一步明确：「暂时不评估，先放着」** —— 即**主动搁置**（不删除方案、不归档，文件保留在 `docs/评估报告/`），等用户主动提起再评估。
