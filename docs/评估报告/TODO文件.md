...
| 🆕 `演示录像操作手册` | **#48** | 🆕 **2026-09-11 新增**：把"**模拟客户行为 → 在 Sentinel / SkyWalking 上看见数据量与变化 → 录像展示**"这件事**照着做一遍**的操作手册。含：**在哪执行**（脚本在**新机**、面板在本机浏览器走隧道）· 前置条件清单 · 隧道与三面板准备 · **两档录像步骤**（① 浏览档 = pass 曲线 ② 限流档 = block 曲线）· **预期数字**（浏览 20/50/100 → **69.0 / 97.2 / 99.6 RPS**，p99 539/976/2466ms，失败 0；限流靶子 `adminLogin` **被限流 93.26%**）· **拐点讲法** · **分镜台词** · 常见疑问（"面板里有的服务没曲线正常吗"）· 收尾回填。**脚本与限流靶子均已实测**，只差按录制键 |
