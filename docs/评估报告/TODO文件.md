...
| 🆕 **已决策待实施** | **#32** | **AI 导购 Agent（Function Calling + ReAct）** —— 前置 **#58 已完成并部署**；**P0 已上线并验证**（`09d22b1`/`0fe1a1e`，生产开关 `true`）；**P1 代码已完成待部署**（流式 Agent + compare/get_stock + Redis 审计，50 项测试全绿）。📖 见 [[TODO第三批实现与原理-1]] §3.6/§3.7 |
...
| 文档 | 说明 |
|---|---|
| `AI导购Agent升级方案` | ✅ **2026-09-10 用户拍板「做」**：P0（最小 Function Calling）→ P1（完整单 Agent）分期；方案已敲定（含 **6 条实施前代码校正** + 文件级执行清单 + 验证 + 回滚）。对应 TODO **#32** |
...
### 32. 【AI】AI 导购升级 Agent（Function Calling + ReAct）（**✅ 已决策实施 2026-09-10**，P0 → P1 分期）
