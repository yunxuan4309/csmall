...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-10（**首次文档归档整理完成**：**16 个已执行完毕的文档移入 `docs/归档/`**；索引表重构为「现存 15 + 归档 16」；**16 处相对路径引用同步修正**；**7 处过期状态头修正**；AI 导购 Agent（#32）状态澄清为"**待决策**"而非"不执行"。同日早些：新增「📚 方案文档索引」并确立"新增文档必须登记"纪律；新登记 #57。此前 09-09：跨机集群阶段 0~5 全部完成并归档 [[TODO已完成]] §十三）
...
> **范围**：`docs/评估报告/` **现存 15 个** + `docs/归档/` **本次移入 16 个**（2026-09-10）。其他目录（`问题解决/`、`运维/`、`归档/`、`面试准备/`）见 [[项目上下文文档]] 附录 B。
