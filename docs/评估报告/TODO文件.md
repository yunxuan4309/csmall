...
| **#50** | **新机哨兵端口收窄**（bind 私网 IP / 加 requirepass） | 🟡 **P3 可选加固（2026-09-09 巡检发现）**：新机 5 个集群成员用 `network_mode: host` → 哨兵监听 `0.0.0.0:26379-26381`（**含公网网卡**）+ `protected-mode no` + **无 requirepass**，目前**仅靠安全组兜底**（公网只开 22/80/443）。哨兵可返回主库地址、可触发 failover，暴露危害大于普通 Redis。**方案**：① 哨兵 conf 改 `bind 172.29.193.240`（最小改动，与老机口径一致）；或 ② 加 `requirepass` + 客户端配 `spring.data.redis.sentinel.password`。replica(6380) 已有 `requirepass`，可一并收窄。→ 见 [[跨机集群实施执行清单-2026-09-09]] §6.5.8 |
...
1. **端口放行**：3306/6379 绑 `127.0.0.1` 是第一批安全加固的副作用 → 跨机不可达（**实测**新机→老机 3306/6379 不通，22/80/8848/5672/8091/9200/10007 通）；改绑**宿主私网 IP** `172.29.193.239`（不是 `0.0.0.0`）→ 公网不可达；**实测无需改安全组**（同安全组内网互通不受公网规则限制）
