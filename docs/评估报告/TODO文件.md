...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-09（✅ #46 nacos 数据卷挂载已完成并归档至 [[TODO已完成]]，踩坑明细见 [[TODO第三批实现与原理]] §一；本文件仅保留**未完成 / 暂缓 / 评估 / 第三批**事项）
...
> **进行中**：🟢 第三批（企业级演进 + 学习）。**✅ #46 nacos 数据卷挂载已完成（2026-09-09，含卷名前缀坑，见 [[TODO第三批实现与原理]] §一）**。时间紧迫时**按下方「🧭 推荐执行路线」取舍**：P0=#47（运维消雷）→ P1=#4/#5-P1（面试主菜）→ P2=#9/#14-P2/#40 → P3=其余只讲认知。
...
| **#47** | **数据库备份恢复演练**（#29 收尾，2026-09-08 记录，低优先） | 运维项：#29 备份已上线（cron 每日 02:30），但 TODO 铁律"**备份没验证过 = 没有备份**"——尚未做恢复演练。待执行：`zcat /data/csmall/backup/cs_mall_*.sql.gz` 改写库名（sed 加 `_vfy` 后缀）导回**临时库**验证 39 表/抽样行数，验证完删临时数据，不碰生产。与 #46 同为运维验证项（#46 已完成，剩 #47） |
...
**执行建议（时间紧张时）**：
1. **先做 #47**（半小时，纯运维，消除"备份未验证"的雷；#46 已于 2026-09-09 完成）
