...
| `TODO第三批实现与原理-1` | 🆕 **补册 1（2026-09-10 新建）**：**#32 Agent + #48 模拟数据** 的设计与**技术选型过程**（Spring AI 评估 / Boot 升级影响面 / 模型配置可配化）+ **10 格实测实验**。因正册 [[TODO第三批实现与原理]] 已 946 行过长，按用户要求另开一册，**正册不动、两册互链** |
