...
| #14-P2 | Redis 主从防数据 - P2 配置层（min-replicas-to-write 1，随 #9 主从哨兵） | ⏳ **唯一剩余的主线项**：`min-replicas-to-write` 实测仍为 `0`；演练已证明脑裂窗口真实存在（10.9s），Redis 官方文档对同一场景给出该补丁 → 详见 [[跨机集群实施执行清单-2026-09-09]] §5.9/§6.4 |
