...
> ⚠️ **计数口径**：本目录 `ls` 会显示 **21 个 `.md`** = **正式文档 20 个** + **1 个 `*-副本.md`**（`TODO第三批实现与原理-副本.md`，状态纠偏用的**工作副本，用户保留备用**）。
> 已删除的是第一/二批的副本（`TODO第一批/第二批实现与原理-副本.md`，2026-09-10 切割完成后按用户要求删除）。已在 `.gitignore` 加 `*-副本.md`。
> **做"定期交叉核对"时请排除 `*-副本.md`**，否则会误判数量（本表口径 = 20 = 🟡8 + 🟢3 + ✅4 + ⚪5；2026-09-11 复核修正，原记 19/7）。
> 🆕 **2026-09-10 新增**：`TODO第三批实现与原理-1.md`（第三批**补册 1**：Agent + 模拟数据 + 技术选型过程）、**`AI导购Agent实现详解.md`**（#32 Agent **实现说明书**：架构/类/取舍/排障/47 项证据/面试底稿）
> 🆕 **2026-09-11 新增**：**`商品与秒杀扩容方案.md`**（#63 ES mapping 前置修复 + 商品 20→60 + 秒杀 6→12 的**已决策实施方案**）、**`演示录像操作手册.md`**（**#67 之"录像"**的操作手册）
