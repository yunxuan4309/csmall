...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-09（✅ #46 nacos 数据卷挂载 + #47 数据库备份恢复演练均已完成并归档至 [[TODO已完成]]，明细见 [[TODO第三批实现与原理]]；**「🚀 第三批执行清单」= 跨机集群主线阶段 0~5 已全部完成**（#4 秒杀双实例 + #9 Redis 主从哨兵 → 已归档 [[TODO已完成]] §十三），**唯一剩余主线项 = #14-P2**（已运行时开启，持久化待 sudo 落盘）；新发现待办 #51/#53；本文件仅保留**未完成 / 暂缓 / 评估 / 第三批**事项）
...
> **进行中**：🟢 第三批（企业级演进 + 学习）。**✅ #46 nacos 数据卷挂载 + #47 数据库备份恢复演练均已完成（2026-09-09，见 [[TODO第三批实现与原理]]）**；**当前主线 = 跨机集群（下方「🚀 第三批执行清单」阶段 0~6，阶段 0/A/2/3/3.5/**3.6** 已完成，**下一步 = 阶段 4 秒杀双实例**）**。时间紧迫时**按下方「🧭 推荐执行路线」取舍**：P1=#4/#5-P1（面试主菜）→ P2=#9/#14-P2/#40 → P3=其余只讲认知。
...
| #14-P2 | Redis 主从防数据 - P2 配置层（min-replicas-to-write 1） | 🟡 **已开启（仅老机主库，2026-09-09 21:45）→ 持久化待做**：老机主库 `CONFIG SET min-replicas-to-write 1` + `min-replicas-max-lag 10`（实测写正常、复制 `lag=0`）。**⚠️ 新机从库明确【不开】**——该参数对被提升为主库的从库同样生效，而切换后新主库手里没有从库 → **会拒绝所有写**（老机不恢复就一直不可写），代价远大于收益。<br>**代价（主库侧）**：只有一个从库 → **新机不可用时老机拒写**；停新机前先 `CONFIG SET min-replicas-to-write 0`（"演练开关"）。**持久化**：`CONFIG REWRITE` 在单文件 bind mount 上必然失败（`Resource busy`）→ 需用户 sudo 手工追加两行到 `/data/csmall/redis/redis-master.conf`（**无需重启 Redis**）；仓库模板已同步 → 见 [[跨机集群实施执行清单-2026-09-09]] §5.9/§6.4/§7.9 |
...
| **#51** | **容器 restart 策略（可用性风险）** | 🔴 **P2（2026-09-09 巡检发现；与集群无关，但比集群问题更严重）**：老机 21 个容器实测 restart 策略 = **14 个 `no` + 6 个 `on-failure` + 1 个 `unless-stopped`** → **老机一旦重启（系统重启 / `systemctl restart docker`），20/21 个容器不会自动恢复**（`on-failure` 只在**非零退出**时重启，而 daemon 停机时容器以 0 退出；`no` 永不重启）。老机已连续运行 41 天，所以从未暴露。**方案**：① 首选 compose 全量改 `restart: unless-stopped`（主机重启也能自恢复）→ 逐个 `up -d --force-recreate`（低峰，注意 ES 分片，见 §纪律 6）；② 或加 systemd 单元，在 docker 启动后执行 `docker compose up -d`。⚠️ 改策略必须 recreate 容器 → 属变更窗口事项。 |
...
### 🚀 第三批执行清单：跨机集群主线（2026-09-09 定稿，**阶段 0~5 已完成，仅剩 #14-P2**）
