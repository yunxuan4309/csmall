...
| 🔴 **可用性风险** | **#51** · **#53** · **#61** | restart 改 `unless-stopped`（老机 20 + 新机 5，需 recreate）· 网关重试 / 优雅下线兜底（需重启网关窗口）· **外部端到端探活（防"坏了 24 小时没人知道"）** |
| 🔴 **安全** | **#55（P1）** · **#54** · **#56** | SSH 允许密码 + root 登录（两台）· RabbitMQ 仍 `guest/guest` · 公开仓库暴露 IP/拓扑（当前选择"接受"） |
| 🟠 **待决策（方案已出）** | **#31** | 生产开向量检索（🔴前置 **#59** 硅基流动充值） |
| 🆕 **已决策待实施** | **#32** | **AI 导购 Agent（Function Calling + ReAct，P0 → P1）** —— 前置 **#58 已于 2026-09-10 完成并部署**；**P0 代码 + 部署均已完成并验证**（`09d22b1` + `0fe1a1e`，离线测试 26 项全绿，生产两阶段验证通过，**当前开关 = `true`**）；剩余 **P1**（工具扩展 / Redis 动作审计 / 流式 Agent）。📖 见 [[TODO第三批实现与原理-1]] §3.6 |
...
> **范围**：`docs/评估报告/` **现存 17 个**（见下方 📍 A）+ `docs/归档/` **本次移入 16 个**（📦 2026-09-10 已移至文末「📦 已完成条目归档区 · 四」，同处还有 ✅ 状态头修正 7 个）。其他目录（`问题解决/`、`运维/`、`归档/`、`面试准备/`）见 [[项目上下文文档]] 附录 B。
...
> 🆕 **2026-09-10 新增**：`TODO第三批实现与原理-1.md`（第三批**补册 1**：Agent + 模拟数据 + 技术选型过程）
...
| `TODO第三批实现与原理-1` | 🆕 **补册 1（2026-09-10 新建）**：**#32 Agent + #48 模拟数据** 的设计与**技术选型过程**（Spring AI 评估 / Boot 升级影响面 / 模型配置可配化）+ **10 格实测实验**。因正册 [[TODO第三批实现与原理]] 已 946 行过长，按用户要求另开一册，**正册不动、两册互链**。🆕 **同日扩充**：**§3.6 P0 实际实现**（与计划骨架的 3 处偏差如实标注）+ **§八 小插曲**（nginx 全站 502 抢修 / 前端 conf 双向漂移，均链到 [[问题解决--服务注册与网关路由]]）|
...
| **P1 完整单 Agent** | ⏳ 未开始 | 工具扩展（compare/get_stock）+ Redis 动作审计 + **流式 Agent（P0 的 Agent 只走 `/ai/chat/send`，SSE 仍旧路径 —— 原方案 S5「最后一轮复用 SSE」顺延到 P1）** |
