...
> - 🟡 **① 录像**仍暂缓；🔴 **#68** 仍待"造数前 dump + 文件名登记"（**每日 02:30 的 cron dump 一直在跑**，缺的是**按批次的那一次** → 见 §68 正文）
>
...
| **#68** | 🟡 **已执行一次（2026-09-12 14:52，`cs_mall_20260912_1452.sql.gz`）**，但**规则未固化** → 目标 = "每次写操作前 dump + 文件名写进该批次"（每日 02:30 的 cron dump 一直在跑，49~57 KB/秒级） | 🟡 P2（建议加 `--require-dump` 强制） | §68 |
...
> 📌 **本次 dump 不属于任何批次**（当前没有待跑的批次）⇒ 定位是"**整库兜底快照**"；以后按批次登记。
