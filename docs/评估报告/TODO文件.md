...
> ⚠️ **计数口径**：本目录 `ls` 现显示 **17 个 `.md`** = **正式文档 17 个** + **0 个 `*-副本.md`** —— 两份工作副本（`TODO第三批实现与原理-副本.md`、`TODO第三批实现与原理-1-副本.md`）已于 **2026-09-11 归档到 `docs/归档/`**（切割 + 循环检查全部通过后，按用户"最后归档副本"的要求执行；它们是 gitignore 的本地备份，`Move-Item` 而非 `git mv`）。
> 已删除的是第一/二批的副本（`TODO第一批/第二批实现与原理-副本.md`，2026-09-10 切割完成后按用户要求删除）。已在 `.gitignore` 加 `*-副本.md`。
> **做"定期交叉核对"时本目录已无 `*-副本.md`**（口径干净：**17 = 🟡8 + 🟢1 + ✅2 + ⚪6**；2026-09-11 一天内**五次变化**：19/7 → 20/8 → 归档 1 篇 + 新建 1 篇 → 归档 3 篇 → **20 → 17**；同日**《TODO第三批实现与原理》切割为薄索引**（🟢 → ⚪，总数不变）；同日**两份 `-副本` 移入 `docs/归档/`** → 本目录 `ls` **19 → 17**）。
