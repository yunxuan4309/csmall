...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-08（第二批全部完成并归档至 [[TODO已完成]]。本文件仅保留**未完成 / 暂缓 / 评估 / 第三批**事项，含运维验证项 #46 nacos 卷挂载、#47 备份恢复演练）
...
> **当前主攻**：🟢 第三批（企业级演进 + 学习）。时间紧迫时**按下方「🧭 推荐执行路线」取舍**：P0=#46/#47（运维消雷）→ P1=#4/#5-P1（面试主菜）→ P2=#9/#14-P2/#40 → P3=其余只讲认知。
...
| **#47** | **数据库备份恢复演练**（#29 收尾，2026-09-08 记录，低优先） | 运维项：#29 备份已上线（cron 每日 02:30），但 TODO 铁律"**备份没验证过 = 没有备份**"——尚未做恢复演练。待执行：`zcat /data/csmall/backup/cs_mall_*.sql.gz | docker exec -i csmall-mysql mysql -uroot -p'<密码>'` 导回**临时库**（或先导临时实例）验证可用性，验证完删临时数据，不碰生产。与 #46 同为"有空再做"的运维验证项 |
...
|---|---|---|---|---|
| 🔴 **P0 必做** | **#46** | nacos 数据卷挂载重启 | **运维安全底线**：当前 nacos 数据在容器可写层，任何重建即丢（含认证用户/规则）；且**下次动 nacos 前必须做**——不做=埋雷。成本 10 分钟 | ~0.5h |
| 🔴 **P0 必做** | **#47** | 数据库备份恢复演练 | **运维安全底线**：备份已配但没验证过=等于没有；导临时库验证一次即可，成本极低 | ~0.5h |
...
**执行建议（时间紧张时）**：
1. **先做 #46 + #47**（各半小时，纯运维，消除两个"雷"：nacos 数据可丢 + 备份未验证）
