...
> ⚠️ **计数口径**：本目录 `ls` 会显示 **19 个 `.md`** = **正式文档 17 个** + **2 个 `*-副本.md`**（`TODO第三批实现与原理-副本.md`、`TODO第三批实现与原理-1-副本.md`，**状态纠偏 / 比对用的工作副本，用户保留备用**）。
> 已删除的是第一/二批的副本（`TODO第一批/第二批实现与原理-副本.md`，2026-09-10 切割完成后按用户要求删除）。已在 `.gitignore` 加 `*-副本.md`。
> **做"定期交叉核对"时请排除 `*-副本.md`**，否则会误判数量（本表口径 = **17 = 🟡8 + 🟢1 + ✅2 + ⚪6**；2026-09-11 一天内四次复核：19/7 → 20/8 → 归档 1 篇 + 新建 1 篇 → 归档 3 篇 → **20 → 17**；同日**《TODO第三批实现与原理》切割为薄索引** → 只是**从 🟢 迁到 ⚪**，总数不变）。
