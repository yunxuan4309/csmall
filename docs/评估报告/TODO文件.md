...
> **创建日期**: 2026-05-13
> **最后更新**: 2026-09-10（**三类改动**：① **首次文档归档整理完成** —— 16 个已执行完毕的文档移入 `docs/归档/`，索引表重构为「现存 **16** + 归档 16」，16 处相对路径引用同步修正，7 处过期状态头修正；② **第一批/第二批「实现与原理」状态纠偏** —— 逐条 SSH 实测 + 代码核实，修掉 Swap/端口/#29 备份/#13/#2+#34/#5.4.5「待部署」等过期表述；③ **新登记 #58**（AI 依赖已公告停用的 `deepseek-chat`）+ **#51 两机复核**（官方文档证实 `on-failure` 不扛 daemon 重启，老机 20/21、**新机 5/5** 不恢复）。同日另：新增「📚 方案文档索引」并确立"新增文档必须登记"纪律；新登记 #57；AI 导购 Agent（#32）状态澄清为"待决策"而非"不执行"、后明确为「暂时不评估，先放着」。此前 09-09：跨机集群阶段 0~5 全部完成并归档 [[TODO已完成]] §十三）
...
> 📌 **用户决定（2026-09-10）**：这 5 个属「问题解决文档」，**待用户细分内容后再处理** → 本次保持标记、**不移动**。
