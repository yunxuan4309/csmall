...
> - → **本条目已从正文整段移入文末「📦 已完成条目归档区」**；归档明细 [[TODO已完成]] §十六（P0）/ **§十七（P1 + 加固）**、实现说明书 [[AI导购Agent实现详解]]
>
...
> **范围**：`docs/评估报告/` **现存 19 个**（见下方 📍 A）+ `docs/归档/` **2026-09-10 移入 16 个**（📦 2026-09-10 已移至文末「📦 已完成条目归档区 · 四」，同处还有 ✅ 状态头修正 7 个）。其他目录（`问题解决/`、`运维/`、`归档/`、`面试准备/`）见 [[项目上下文文档]] 附录 B。

### 📍 A. `docs/评估报告/` 现存 19 个

> ⚠️ **计数口径**：本目录 `ls` 现显示 **20 个 `.md`** = **正式文档 19 个** + **1 个 `*-副本.md`**（**`TODO第三批实现与原理-1-副本.md`** —— **2026-09-11 为"补册 1 瘦身"新建的比对底稿**，切前 966 行、md5 与正本一致；**待本次切割复核通过后按"最后归档副本"的流程移入 `docs/归档/`**）。此前两份旧副本已于同日归档到 `docs/归档/`。
> 已删除的是第一/二批的副本（`TODO第一批/第二批实现与原理-副本.md`，2026-09-10 切割完成后按用户要求删除）。已在 `.gitignore` 加 `*-副本.md`。
> **做"定期交叉核对"时请排除 `*-副本.md`**（口径干净：**19 = 🟡9 + 🟢1 + ✅2 + ⚪7**；2026-09-11 一天内**六次变化**：19/7 → 20/8 → 归档 1 篇 + 新建 1 篇 → 归档 3 篇 → **20 → 17**；同日**《TODO第三批实现与原理》切割为薄索引**（🟢 → ⚪，总数不变）；同日**两份旧 `-副本` 移入 `docs/归档/`** → `ls` **19 → 17**；同日**为补册瘦身新建 1 份 `-副本`** → `ls` **17 → 18**；🆕 **同日按「任务」拆分 1 篇为 2 篇**（Python 造数 / AI 压测），**原篇原地改薄索引** → `ls` **18 → 20**、正式文档 **17 → 19**（🟡8→9 · ⚪6→7））
...
| 🆕 `Python模拟数据与数据隔离方案` | **#48** ✅ / **#67**（① ② ④ ⑤ ⑥） | 🆕 **2026-09-11 拆分**（原《Python模拟数据与AI并发测试方案》的**第一层**，**986 行**）：**造数 + 数据隔离 + 标识契约 + 清理算法 + 脚本骨架 + 可观测展示与录像 SOP** 的完整方案正文。状态：🟢 **第一层已完成并交付**（5 次真跑校准 / 回填校验通过 / 清理演练 / 两档压测标定 / **#66 面板可见性修复**）；⏳ 剩余 **① 录像**（脚本 + Runbook 就绪）**② 正式造数** **④ `--with-seckill`** **⑤ Redis 精确清理** **⑥ `sim_baseline` 基线比对**。🧩 **已提炼** → [[问题解决--生产造数的数据隔离与复核方法]]。⚠️ **未完成前不得归档** |
| 🆕 `AI并发测试方案` | **#67 之 ③** | 🆕 **2026-09-11 拆分**（原《Python模拟数据与AI并发测试方案》的**第二层**，**249 行**）：**内网 mock LLM（SSE 分片 + `tool_calls` 工具轮，`ThreadingHTTPServer`）+ 生产压测执行剧本（Nacos 热改/备份/中止阈值/回滚点）+ 实施前复核 D1~D5·D8a·D8b + 两项决策**（生产 + 临时放开限流 · **Agent 双链路各压一遍**）+ 压测侧纪律 10 项 + 压测/AI 话术。🔴 **状态：完全未开始**（`mock_llm.py` 未写 · compose `AI_API_BASE_URL` 未透传 · Nacos 规则未放开）→ **做完前不得归档**。🧩 **尚未提炼**（AI 侧无「一类问题」文档） |
...
> | **#67**（= #48 剩余 6 项）关闭后 | [[Python模拟数据与数据隔离方案]] + [[AI并发测试方案]] | 它们是 #67 的方案正文（**造数侧在 A / AI 压测侧在 B**；2026-09-11 由原《Python模拟数据与AI并发测试方案》**按任务拆分**而来）（录像 / 正式造数 / 第二层 AI 压测 / `--with-seckill` / Redis 精确清理 / 基线比对全在里面）；收口后方法已提炼为 [[问题解决--生产造数的数据隔离与复核方法]] |
...
| 📇 `Python模拟数据与AI并发测试方案` | 📇 **2026-09-11 切割为薄索引**（**1170 → 58 行**）：因**一份文档里装了两个任务**（第一层造数 / 第二层 AI 并发压测）→ 拆为上述两册（[[Python模拟数据与数据隔离方案]] + [[AI并发测试方案]]）。**保留文件名**使全仓库 **31 处** `[[引用]]` **零失效**（与三册「实现与原理」同一做法）。含 **切割索引表**（原 §〇/§〇.1/§〇.2/§一~§八 → A/B **逐段去向**）+ **§二 未完成部分指针**（#67 六项，🔴 不许提前提炼） |
...
| `问题解决--生产造数的数据隔离与复核方法` | 生产造数两条铁律：**可逆**（登记表是唯一清理依据）+ **可证**（校验必须覆盖"没登记的表"）| [[Python模拟数据与数据隔离方案]]（§2.2 隔离 / §〇.1 D6·D7·D8c·D9 / §〇.2 §F 复核方法 / §G 校准实测）、[[TODO第三批实现与原理-1]] §十三 |
