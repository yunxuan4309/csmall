...
| **#50** | **新机哨兵加认证**（requirepass / ACL；bind 私网 IP 为次要） | 🟡 **P3 可选加固（2026-09-09 巡检发现）**：新机 5 个集群成员用 `network_mode: host`（绕过 docker 端口映射），哨兵监听 `0.0.0.0:26379-26381` 且**无 `requirepass`** → 任何能连上的人可 `SENTINEL FAILOVER` / `SENTINEL SET` / `SENTINEL MONITOR`（指向假主库），**哨兵是拓扑控制面，危害大于普通 Redis**。**有效方案**：给哨兵加 `requirepass`（或 ACL）+ 11 个客户端配 `spring.data.redis.sentinel.password`。⚠️ **无效方案（已修正）**：`bind 172.29.193.240` **挡不住公网**——阿里云公网 IP 是 NAT、不在网卡上，绑 0.0.0.0 与绑私网 IP 公网可达性等价，**公网闸门只有安全组**；bind 私网 IP 仅作监听面收敛。`protected-mode no` 也不是原因（显式 bind 下该开关不生效）。实证：公网探测 `47.109.70.197:26379/6380`、`8.156.77.197:6379/3306/10087` 全 blocked，仅 `:80` 通。→ 见 [[跨机集群实施执行清单-2026-09-09]] §6.5.8 |
