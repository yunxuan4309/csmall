...
| #14-P2 | Redis 主从防数据 - P2 配置层（min-replicas-to-write 1，随 #9 主从哨兵） | 🟡 **运行时已开启（2026-09-09 21:45）→ 持久化待做**：老机主库与**新机从库**均已 `CONFIG SET min-replicas-to-write 1` + `min-replicas-max-lag 10`（实测写操作正常、复制 `lag=1`）。⚠️ **仅运行时生效**：`CONFIG REWRITE` 在单文件 bind mount 上必然失败（`Resource busy`，与哨兵同源坑）→ 需**用户 sudo 手工追加两行**到 `/data/csmall/redis/redis-master.conf` 与 `redis-replica.conf`（**无需重启 Redis**，下次重启自动生效）。**代价**：只有一个从库 → 从库不可用时**主库拒写**（防脑裂丢数据的必要取舍）→ 见 [[跨机集群实施执行清单-2026-09-09]] §5.9/§6.4 |
