...
> **范围**：`docs/评估报告/` **现存 20 个**（见下方 📍 A）+ `docs/归档/` **本次移入 16 个**（📦 2026-09-10 已移至文末「📦 已完成条目归档区 · 四」，同处还有 ✅ 状态头修正 7 个）。其他目录（`问题解决/`、`运维/`、`归档/`、`面试准备/`）见 [[项目上下文文档]] 附录 B。

### 📍 A. `docs/评估报告/` 现存 20 个

> ⚠️ **计数口径**：本目录 `ls` 会显示 **22 个 `.md`** = **正式文档 20 个** + **2 个 `*-副本.md`**（`TODO第三批实现与原理-副本.md`、`TODO第三批实现与原理-1-副本.md`，**状态纠偏 / 比对用的工作副本，用户保留备用**）。
> 已删除的是第一/二批的副本（`TODO第一批/第二批实现与原理-副本.md`，2026-09-10 切割完成后按用户要求删除）。已在 `.gitignore` 加 `*-副本.md`。
> **做"定期交叉核对"时请排除 `*-副本.md`**，否则会误判数量（本表口径 = 20 = 🟡8 + 🟢3 + ✅4 + ⚪5；2026-09-11 两次复核：先修正 19/7 → 20/8，同日**归档**《集群化与配置中心迁移方案》并**新建**《配置中心迁移方案》，总数与分类数均不变；新增 1 个 `原理-1` 副本）。
