...
| 20 | **AI 重排 Read timed out：reasoning 模型慢 + timeout 太短**（2026-09-08） | key 修复后 AI 意图提取/查询扩展成功，但最后一步 **AI 重排序 Read timed out**（约 18s 超时）→ 降级 ES 原始排序（商品能返回但无 AI 解释）。根因：`deepseek-v4-flash` 是 **reasoning 模型**（响应含 `reasoning_content`，先思考后回答），完整生成常需 15-30s；而 `cooxiao.ai.timeout` 原 15s（application.yml，test/prod 继承），**远不够**。**修复**：application.yml `timeout: 15000 → 60000`（prod 的 cooxiao.ai 段未覆盖 timeout，自动继承；此前误以为 prod `timeout: 10000` 是 AI 超时，实际那是 dubbo consumer 段）。**经验**：① reasoning 模型（deepseek-v4-flash/deepseek-reasoner）比 chat 模型慢一个量级，LLM 调用超时要按"完整生成"估算而非"首 token"；② 看 yml 超时要先确认所属配置段（dubbo.consumer vs cooxiao.ai）|
