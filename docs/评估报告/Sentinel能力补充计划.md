# Sentinel 能力补充计划（评估+计划）

> **创建**: 2026-08-26
> **状态**: 🟡 P0 代码批已完成（2026-09-08）——审计发现原"规则持久化 Nacos + 代码双保险"记载与事实不符（Nacos 规则全空、秒杀本地规则被空配置覆盖=限流失效），已按"Nacos 统一管理"重构：order/sso 加 sentinel-datasource-nacos + datasource 配置、秒杀代码规则保留为宕机兜底、规则 JSON 入库 `deploy/docker/sentinel/`（事实来源）、sso 补 dashboard env。**待服务器执行**（建 Nacos 规则 + 重建 order/sso/seckill），执行清单见 [[Sentinel部署执行清单-2026-09-08]]。P1 热点参数限流未做（随秒杀集群化评估）；P2 集群流控未做。
> **背景**: 2026-08-26 对项目 Sentinel 使用程度做全面评估（代码实证），
> 当前只用了"基础三件套"（流控 + 规则持久化 Nacos + 控制台），
> 企业级高级能力（热点限流/系统保护/授权/集群流控）均未使用。
> **关联**: [[集群化与配置中心迁移方案]]（集群流控与之配套）、[[Sentinel部署执行清单-2026-09-08]]（P0 执行清单）
