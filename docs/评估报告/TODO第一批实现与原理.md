...
| §七 | 服务器部署执行清单 | **保留在本文件 §三**（一次性执行手册，非"一类问题"）|
| §八 | 第一批通用面试话术 | **保留在本文件 §五** |
| §九 | 实战执行记录（3 个坑）| §9.1 结果快照 → **保留在本文件 §六**；§9.2 RDB→AOF 丢数据 + §9.4 conf 权限 → [[问题解决--Redis加固与主从数据一致性]] §四；§9.3 gateway 启动竞态 → [[问题解决--容器构建与编排卫生]] §三 |
...
> 📌 ~~**本表是"执行前"视角**（"服务器待做"列即当时的待办）。**7 项已全部执行完毕**，结果见 §九；表中 `127.0.0.1` 等细节的现状变化见 §3.3 ⚠️ 与 §六 ⚠️。~~（⚠️ 2026-09-12：本行与下方紧邻的修正版**重复**，且引用的 §九/§3.3 在本文件已不存在 ⇒ **以修正版为准，本行作废**）
