...
| 多实例并发对账？ | ✅ **已用分布式锁收口**：秒杀已于 2026-09-10 完成双实例（老机 + 新机各一个），对账/重试任务改用 `RedisLockUtils`（SET NX + Lua CAS 释放）跨 JVM 互斥 —— `MessageRetryTask` 抢不到锁即跳过（`MessageRetryTask.java:51-55`）、`SeckillReconcileTask` 运行期/凌晨两把锁同理（`SeckillReconcileTask.java:75-79`），并通过**受控争用验证**（本地双实例锁验证报告，已归档）|
