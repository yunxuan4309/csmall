...
> ✅ **执行后回填（R7 于 2026-09-07 执行 / 2026-09-10 复核）**：上面这份"available 仅 2.0G、无 Swap、mem_limit 全为 0"的**执行前快照已不再成立** —— **21 容器 `mem_limit` 全部落地**、**Nacos 堆降至 512m**、**Sentinel 限堆 256m**、**Swap 2G 已上**（`/swapfile` + fstab）；实测 **available 2.0G → 3.5~3.9G**（[[TODO已完成-明细-2026-09-12]] §R7 记 3.9G；后续复核 3.5~3.6G 属正常波动）。
...
> ✅ **执行后回填（2026-09-07 执行 / 2026-09-10 复核）**：上表四项**全部落地** —— ① 全 **21 容器** `mem_limit`（`docker update --memory` 即时生效 + 已写回 compose 持久化）；② Nacos `JVM_XMS/XMX=512m`、③ Sentinel `-Xmx256m` 实测生效；④ **Swap 2G 已上**（复核实测 `/swapfile 2G` 且写入 fstab，由 ecs-user sudo 执行）。**available 实测 2.0G → 3.5~3.9G**。依据：[[TODO已完成-明细-2026-09-12]] §R7 · [[TODO第一批实现与原理]] 状态头。
