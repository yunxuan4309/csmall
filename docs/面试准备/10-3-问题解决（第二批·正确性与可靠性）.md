...
> ✅ **2026-09-12 按基线回填（#36 已完成 + #70 A~E 已部署）**：上表三行原是 §3.5 落地前的旧稿，现按实测更正。**#36**（订单 DLX 死信 + requeue 限次）第二批已完成并部署；**#70 A~E** 第三批把库存扣减 MQ 链路收口并上线 —— **AUTO ack**（ack 全交容器、监听器不碰 channel）+ 容器重试 3 次 + **毒消息 reject 进 DLX** + `【MQ死信告警】`告警。⚠️ 副产物（踩坑叙事）：原「读 `x-death` 做限次」方案被实测**证伪**——classic 队列 `basicNack(requeue=true)` 不写 `x-death`、count 恒 0，实际是无限重投。依据：[[TODO已完成]] §六 · [[问题解决--消息可靠性与毒消息治理]] §四。
