# 10-3 面试准备 — 问题解决（第二批 · 正确性与可靠性）

> **用途**: 面试深挖素材 —— "代码层面的坑 / 消息可靠性"
> **覆盖 3 个点**: `#8 时区 bug` / `#23 校验静默失效` / `#36 MQ 毒消息与死信`
> **技术正文来源**: [[问题解决--请求校验与静默失效]]、[[问题解决--消息可靠性与毒消息治理]]（由 [[TODO第二批实现与原理]] 切割而来）
...
### Q6. #8 AI 预算按北京时间结算（时区 bug 的经典案例）

> 来源：[[问题解决--请求校验与静默失效]] §二 · 问题 1
...
### Q7. #23 漏触发接口补 @Validated（审计修正 + 全局异常补全）

> 来源：[[问题解决--请求校验与静默失效]] §三 · 问题 2
...
**延伸发现（AdminController 已改；未做的只是前端 admin.js 残留）**：`AdminController.addAdmin` 是 **GET + DTO 绑定**（非 @RequestBody），其 `AdminAddDTO` 校验注解同样全被注释；本次已恢复其校验（@NotBlank/@Pattern）+ 类级/参数级 @Validated。另核实**前端 admin.js 是废弃残留**（无人引用，真接口在 sso.js），契约不一致不影响线上，已记录。
...
### Q8. #36 MQ requeue 修复 + DLX（含 406 PRECONDITION_FAILED 实战）

> 来源：[[问题解决--消息可靠性与毒消息治理]] §二 · 问题 1
...
> 🔴 **2026-09-12 更正（详见下表）**：这套「读 `x-death` 做限次」**在 classic 队列上不成立** —— `basicNack(requeue=true)` **不会**写 `x-death`（只有"被死信"才写）⇒ count **恒为 0** ⇒ `0 < 3` 恒真 ⇒ **永久重投**。正确做法：**毒消息抛 `AmqpRejectAndDontRequeueException` ⇒ 进 DLX**；**瞬时异常交给容器 `retry.max-attempts=3` 重试**，耗尽后 reject ⇒ DLX。

#### 3.3 疑惑点与原理（面试深挖）

| 疑惑/问题 | 结论 |
|---|---|
| x-death 是什么？ | RabbitMQ 在消息**进入死信**（reject 进 DLX / TTL 过期等）后附加的**投递历史头**。⚠️ **2026-09-12 实测更正**：classic 队列的 `basicNack(requeue=true)` **不写** `x-death`、count **恒为 0** ⇒ **不能拿它做限次重试**（这正是那次"无限重投"的根因）|
...
| manual ack + retry 能共存吗？ | 秒杀模块配了 manual + retry(max-attempts=3) —— ❌ **2026-09-12 实测证伪（#70 D）**：MANUAL 下抛异常**不会 reject** ⇒ 消息**永久 unacked**、DLX 收不到；最终定稿 **AUTO ack**（监听器完全不碰 channel）+ `default-requeue-rejected: false` + `retry.max-attempts: 3` ⇒ 耗尽后 reject 进 DLX |
| 为什么订单不直接学秒杀配 retry？ | 秒杀有本地消息表（`seckill_message_retry` + `MessageRetryTask` 每 5s 重发）兜底；订单**当时没有发送确认、没有重试机制**——两条链路可靠性设计不一致（订单弱）。✅ **2026-09-12 #70 C/D 已补**：订单把 ack/retry 从 yml（**影子配置、不生效**）搬进代码里的 `rabbitListenerContainerFactory`，设为 **AUTO + 重试 3 次** |
| requeue 耗尽后消息去哪？ | ✅ **已实现并部署（2026-09-12；#36 已完成）**：reject ⇒ 进 **DLX** ⇒ 死信消费者 `OrderDlxConsumer` 记录原因 + 打 `【MQ死信告警】`（含消息体原文）+ 人工补偿；秒杀侧同款已于同日落地（#72，死信靠 **broker policy** 施加，避免 `PRECONDITION_FAILED`） |

> ✅ **2026-09-12 按基线回填（#36 已完成 + #70 A~E 已部署）**：上表三行原是 §3.5 落地前的旧稿，现按实测更正。**#36**（订单 DLX 死信 + requeue 限次）第二批已完成并部署；**#70 A~E** 第三批把库存扣减 MQ 链路收口并上线 —— **AUTO ack**（ack 全交容器、监听器不碰 channel）+ 容器重试 3 次 + **毒消息 reject 进 DLX** + `【MQ死信告警】`告警。⚠️ 副产物（踩坑叙事）：原「读 `x-death` 做限次」方案被实测**证伪**——classic 队列 `basicNack(requeue=true)` 不写 `x-death`、count 恒 0，实际是无限重投。依据：[[TODO已完成-明细-2026-09-12]] §六 · [[问题解决--消息可靠性与毒消息治理]] §四。

#### 3.4 面试话术

> ⚠️ **话术前置更正（2026-09-12）**：下面这段里的「`x-death` 限次重试」**已被实测证伪**（classic 队列 requeue 不写 `x-death`、count 恒 0），**正确说法见上表回填行**；此处保留原文以留痕。
...
5. 🆕 **`__TypeId__` 是载荷类型的承重结构（2026-09-12 第⑯批实测补充）**：去掉消息头里的 `__TypeId__` 后，`fromMessage` 只得到 `LinkedHashMap` —— **"仅靠方法参数提示兜底"并不成立** ⇒ **生产端转换器必须写类型头**；死信监听因此要**方法级 + 收原始 `Message`**（#65 的教训，见 [[问题解决--消息可靠性与毒消息治理]]）
...
## 📎 附录：第二批通用面试话术（贯穿主线；原文 [[TODO第二批实现与原理]] §四）
