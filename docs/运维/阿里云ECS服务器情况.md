...
> 创建日期：2026-07-29
> 最后更新：**2026-09-09（新增第二台服务器 csmall-node2 + 跨机连通性实测）**
> 用途：记录本次部署的阿里云 ECS 服务器全部配置信息（**两台**：老机主环境 + 新机跨机集群实验）

---

## 〇、两台服务器总览（2026-09-09 起）

| 角色 | 实例名 | 公网 IP | 私网 IP | 规格 | 用途 |
|------|--------|---------|---------|------|------|
| **老机**（主环境） | launch-advisor-20260729 | **8.156.77.197** | 172.29.193.239 | 4C16G 经济型 e | 21 容器全栈：MySQL 主 / Redis 主 / Nacos / MQ / ES / Seata / Sentinel / SkyWalking / 11 微服务 / 前端 |
| **新机**（集群实验） | csmall-node2 | **47.109.70.197** | 172.29.193.240 | 2C8G 经济型 e | Redis 从 + 3 哨兵 / 秒杀副本(10017) / 造数·压测执行位；**同 VPC 同安全组** |

**网络关系**：同 VPC `vpc-2vcxszplmcrzxluvgdnyr`（172.29.193.0/20），内网互通实测 **ping 0.43ms / 0% 丢包**。
**关键限制**：老机 MySQL(3306)/Redis(6379) 绑 `127.0.0.1`（第一批安全加固）→ **跨机不可达**，需端口放行改造（见 [[跨机集群方案-讨论纪要-2026-09-09]] §7.1）；Nacos(8848)/MQ(5672)/Seata(8091)/ES(9200)/秒杀(10007) 绑 0.0.0.0 → 跨机可达。
**账号体系**：两台均配置 `ai-deepseek`（docker 组、无 sudo，同一把密钥通两台）+ `ecs-user`（人类，sudo）；新机另有独立自定义密码。
...
## 一·二、新机基本信息（2026-09-09 创建）

| 配置项 | 值 |
|--------|-----|
| 实例 ID | i-2vcg7pnk8j6mrwy56jny |
| 实例名称 | csmall-node2 |
| 实例规格 | ecs.e-c1m4.large（经济型 e） |
| vCPU / 内存 | 2 核 / 8 GiB |
| 架构 | X86 计算 — 共享型 |
| 地域 | 西南1（成都）可用区 B |
| 操作系统 | Ubuntu 24.04.4 LTS（安全加固，内核 6.8.0-138） |
| 系统盘 | ESSD Entry 40 GiB |
| 公网 IP | **47.109.70.197** |
| 私网 IP | 172.29.193.240 |
| 主机名 | iZ2vcg7pnk8j6mrwy56jnyZ |
| 公网带宽 | **按使用流量**（5 Mbps 峰值）——集群流量走内网，公网仅 SSH |
| 专有网络 | `vpc-2vcxszplmcrzxluvgdnyr`（老机同 VPC） |
| 交换机 | `vsw-2vcsykcg55qvk4sibip6y` |
| 安全组 | `sg-2vc9i0zxt8gbq18mo8ea`（**老机同安全组** → 内网互信） |
| 登录方式 | 自定义密码（ecs-user） |
| 付费类型 | 包年包月 ¥71.28/月 |
| 到期时间 | 2026-10-09（**需记得续费**） |
| 已装环境 | Docker 29.8.0 + Compose v5.5.1（阿里云 docker 源）；ai-deepseek 用户（docker 组） |
| 用途 | 跨机集群实验：Redis 从 + 3 哨兵 / 秒杀副本(10017) / 造数·压测执行位（CPU 隔离） |
...
> 💡 **两台机器同安全组**：阿里云同安全组内实例默认内网互信 → 跨机访问老机中间件（放行端口后）无需额外安全组规则，只需把端口从 127.0.0.1 改绑内网 IP。
