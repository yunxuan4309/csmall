...
**延伸发现（⚠️ 2026-09-12 口径更正）**：`AdminController.addAdmin` 是 **GET + DTO 绑定**（非 @RequestBody），其 `AdminAddDTO` 校验注解**同样曾被注释**，**本次已恢复**（@NotBlank/@Pattern + 类级/参数级 @Validated）。**仍记 TODO 的是另一件事**：前端 `admin.js` 是废弃残留（无人引用，真接口在 `sso.js`），契约不一致不影响线上。另核实**前端 admin.js 是废弃残留**（无人引用，真接口在 sso.js），契约不一致不影响线上，已记录。
