...
```bash
# 通过接口生成哈希
# ⚠️ 2026-09-12：该调试接口（/admin/sso/hash）已在代码中移除，不要照抄；需要哈希时离线生成
...
| 用户名 | 密码 | 角色 |
|--------|------|------|
| admin | 123456 ✅（2026-09-12 实测登录成功） | 超级管理员 |
| liucs | ⚠️ **不是 123456**（2026-09-12 实测登录失败，`state=400`） | 运营管理员 |
| wangkj | ⚠️ **不是 123456**（与 liucs 同一旧哈希，实测同样失败） | 客服管理员 |
...
3. 添加单元测试覆盖登录流程
4. ~~清理调试用的临时接口（如 `/admin/sso/debug` 和 `/admin/sso/hash`）~~ ✅ **已完成**：两个接口已在代码中移除（见 [[TODO已完成]]）
