...
Android App 拍照上传时返回 `503 Service Unavailable`，但 mall-resource 服务本身正常运行（`systemctl status mall-resource` 显示 active）。（⚠️ 2026-09-12：部署形态已变 —— 现为 Docker Compose，容器名 `csmall-resource`；`systemctl` 是 systemd 时期的说法）
...
mall-resource 是独立服务，pom.xml **没有**引入 `spring-cloud-starter-alibaba-nacos-discovery` 依赖，不会向 Nacos 注册。
> ⚠️ **2026-09-12 更正**：这与现行代码**相反** —— `mall-resource/pom.xml` **已**引入该依赖，且网关路由至今仍是 `lb://mall-resource` ⇒ 后来走的是「补依赖」而不是「改直连」。本节保留为当时的排查记录。而 Gateway 使用 `lb://`（LoadBalance）前缀路由，依赖 Nacos 服务发现，找不到目标地址导致 503。
...
**部署步骤**：重新打包 gateway → 上传 JAR 到服务器 → `sudo systemctl restart mall-gateway`（⚠️ 2026-09-12：现行做法是**容器化重建** —— `cd /data/csmall && docker compose up -d mall-gateway`，jar 落 `/data/csmall/jars/`）
...
MySQL 密码 `Csmall2026!Root` 包含 `!` 字符，在 Bash 中被解释为历史扩展。（⚠️ 2026-09-12 更正：该口令**已失效** —— #24 已于 2026-09-07 把 MySQL root 改为 43 位强随机密码；此处为**历史口令**。现行命令请用 `-p"$MYSQL_ROOT_PASSWORD"` 变量引用）
...
> ⚠️ **2026-09-12 复核**：本节清单里多条路径在仓库中**已不存在** —— `database/08-resource-upload-record.sql`、`docs/android-dev-guide.md`、`docs/android-login-camera-guide.md`、`PROJECT_CONTEXT.md`、`docs/平台基础信息.md`（Android 两份指南疑已并入 [[问题解决--安卓图片上传]]）⇒ **待人工核对后改指现存路径或标注「已删除/已合并」**。
