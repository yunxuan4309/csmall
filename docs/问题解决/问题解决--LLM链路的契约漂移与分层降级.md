...
> **关联（仅互链、不合并）**: [[问题解决--搜索双索引与降级分层]]（该文 §关联 曾预留"未来可并入本类：**#58**、**#32**" —— **本篇就是那次并入**）· [[问题解决--资源治理与流量防护]]（**限流/闸门/预算** 那一半）· [[本地双实例锁验证报告-2026-09-09]]（Mockito 环境限制，与本篇问题 3 同源）· [[问题解决--AI导购模块部署]]（Nacos 双注册 / Dubbo 端口）
