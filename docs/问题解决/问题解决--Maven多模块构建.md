...
- ~~统一所有 service 模块版本为 `1.0.0`~~（✅ **2026-09-12 全仓实测**：含 `<version>1.0.0</version>` 的 pom **只有 1 个** —— `mall-front/mall-front-webapi`（并非 service 模块）；**19 个 `*-service` 模块现均为 `0.0.1-SNAPSHOT`**（继承父 POM）⇒ 该项**从未落地或早已回退**，以下一条「子模块不再指定 version」为准）
