...
> **覆盖**: [[数据库Schema漂移审计]]（本地 MySQL **39 张表** × mall-pojo **32 个实体** × `database/` **34 个 SQL 文件** 三方对比）+ **#57**（服务器是否已执行同款 ALTER 待只读核实）+ 2026-09-11 新增 2 处漂移
...
## 五、2026-09-11 新增的 2 处漂移（**同属 #57 范围，均未追根因**）

> 📌 来自 [[TODO文件]] **#57**（2026-09-11 登记，P3）

| # | 漂移 | 事实 | 状态 |
|---|------|------|------|
| ① | `cs_mall_seckill.seckill_message_retry` | **生产存在（79 行）**，但 `database/` 目录里**没有对应 DDL 文件** | ⚠️ **未追根因** → 待并入 #57 只读核对清单 |
| ② | 生产 `ams_permission` | **多出一列 `value`（全 NULL）**，而 DDL 里**没有该列** | ⚠️ **未追根因** → 待并入 #57 只读核对清单 |

**为什么值得单列**：这两处**方向正好相反**，把本类问题的两种极端都占了 ——
- ① 是**"线上有、仓库没有"**：这张表是**本地消息表**（秒杀 `MessageRetryTask` 每 5 秒 DB 轮询重发），**说明它是在线上"长"出来的**，仓库完全不知道它的结构 → **一旦环境重建（新建库/容灾恢复），这张表会凭空消失**，而重试机制会静默失效。
...
## 六、当前状态与待办：**本地已修复 / 服务器待只读核实（#57，P3）**
...
| **服务器库** | ⚠️ **是否已执行同款 ALTER —— 待只读核实** | [[TODO文件]] **#57** |
...
② 只读查询：查 information_schema（COLUMNS / TABLES），不碰任何数据
      ...
      - 重点补 #57 新增两项：seckill_message_retry 是否存在 / ams_permission 是否有 value 列
...
- **状态源与待办**：[[TODO文件]] **#57**（Schema 漂移只读核对，P3；含 2026-09-11 新增 2 处）；已完成明细见 [[TODO已完成]] §18.1（#63）
