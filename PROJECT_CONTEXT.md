...
- [x] 部署到阿里云 Linux 服务器（已选：4核16G 经济型e 成都节点）
- [ ] 安装 RedisBloom 模块，切换秒杀模块布隆过滤器实现
- [x] 购买域名，配置 HTTP/HTTPS 访问（已购：coolshark-shop.cn，待备案）
- [x] 生产环境配置 (application-prod.yml)（所有模块已创建，敏感信息改用环境变量）
- [ ] 服务器环境搭建（JDK/MySQL/Redis/Nacos/ES/RabbitMQ/Seata）
- [ ] 后端服务部署上线
- [ ] 域名 ICP 备案
- [ ] 前端联调部署
