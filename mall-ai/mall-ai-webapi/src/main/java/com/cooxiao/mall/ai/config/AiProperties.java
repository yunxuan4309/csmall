 ...
 *
 * <h3>模型配置可配化（TODO #58，2026-09-10）</h3>
...
public class AiProperties {
    ...
    /**
     * 商品类问题的**首轮强制调用工具**（默认开）。
     * <p>依据 2026-09-10 生产实测：模型有时会**凭历史对话里的商品作答**（内容也许对，但没有工具结果
     * → **前端没有商品卡片**）。命中商品意图关键词时用 {@code tool_choice=required} 强制检索一次，
     * 后续轮次回到 {@code auto} 由模型自己决定收敛。可用配置关闭（回退为纯 auto）。
     */
    private boolean agentForceFirstTool = true;
